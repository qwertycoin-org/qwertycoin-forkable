...
namespace Common {
    ...
    template <typename T>
    T read (IInputStream &in, size_t uSize)
    {
        T value;
    ...
    public:
        ...
        friend std::ostream &operator<< (std::ostream &os, const ContainerFormatter <T> &formatter)
        {
            ...
            if (!formatter.mContainer.empty()) {
                os << formatter.mContainer.front();

                for (auto it = std::next(formatter.mContainer.begin());
                     it != formatter.mContainer.end();
                     ++it) {
                    os << ", " << *it;
                }
            }
