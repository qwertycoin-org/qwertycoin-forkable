...
/*!
    \class StringBuffer
    \inmodule Common
    \brief 'This is a string of fixed maximum size.
*/
...
public:
    ...
    */
    ...
    /*!
        ...
        ('stringData' != 'nullptr' || 'stringSize' == 0) && 'stringSize' <= 'MAXIMUM_SIZE'.
    */
    StringBuffer(const Object *stringData, Size stringSize)
        : size(stringSize)
    {
    ...
    */
    ...
    explicit StringBuffer(const Object(&stringData)[stringSize])
        : size(stringSize - 1)
    {
        assert(stringData != nullptr || size == 0);
    ...
    */
    explicit StringBuffer(const StringView &stringView)
    ...
    */
    ...
    /*!
        ...
        The behavior is undefined unless 'stringView.size()' <= 'MAXIMUM_SIZE'.
    */
    StringBuffer &operator=(const StringView &stringView)
    {
    ...
    */
    bool beginsWith(const StringView &other) const
    {
        if (size >= other.getSize()) {
            for (Size i = 0;; ++i) {
                if (i == other.getSize()) {
    ...
    */
    ...
    /*!
        ...
        returns index or INVALID if there are no occurences.
    */
    Size find(const StringView &other) const
    {
        if (size >= other.getSize()) {
            Size i = size - other.getSize();
            for (Size j = 0; i >= j; ++j) {
    ...
    */
    StringBuffer &append(const StringView &stringView)
    {
    ...
    */
    ...
    /*!
        ...
        The behavior is undefined unless:
        'index' <= 'size' and 'stringView.size()' <= 'MAXIMUM_SIZE' - 'size'.
    */
    StringBuffer &insert(Size index, const StringView &stringView)
    {
        assert(index <= size);
    ...
    /*!
        Overwrites 'StringBuffer' starting at 'index' with 'stringView',
        possibly expanding 'StringBuffer'. The behavior is undefined unless:
        'index' <= 'size' and 'stringView.size()' <= 'MAXIMUM_SIZE' - 'index'.
    */
    StringBuffer &overwrite(Size index, const StringView &stringView)
    {
        assert(index <= size);
