...
#define BUFSIZE 10000
...
#else
    std::string getNixVersionDisplayString()
    {
        utsname un;
    ...
    std::string getSpecialFolderPath (int iNFolder, bool bIsCreate)
    {
        namespace fs = boost::filesystem;
        char psz_path[MAX_PATH] = "";

        if (SHGetSpecialFolderPathA(NULL, psz_path, iNFolder, bIsCreate)) {
            return psz_path;
        }
    ...
    std::string getDefaultDataDirectory ()
    {
        ...
        // Windows
        config_folder =
            getSpecialFolderPath(CSIDL_APPDATA, true) + "/" + QwertyNote::CRYPTONOTE_NAME;
                                 ...
                                 const std::string &cReplacedName)
    {
        ...
        DWORD attributes = ::GetFileAttributes(cReplacedName.c_str());
        if (INVALID_FILE_ATTRIBUTES != attributes) {
            ::SetFileAttributes(cReplacedName.c_str(), attributes & (~FILE_ATTRIBUTE_READONLY));
        }

        bool ok = 0 != ::MoveFileEx(cReplacementName.c_str(),
                                    cReplacedName.c_str(),
                                    MOVEFILE_REPLACE_EXISTING);
        code = ok ? 0 : static_cast<int>(::GetLastError());
#else
        bool ok = 0 == std::rename(cReplacementName.c_str(), cReplacedName.c_str());
        code = ok ? 0 : errno;
#endif
    ...
    bool directoryExists (const std::string &cPath)
    {
        ...
        return boost::filesystem::is_directory(cPath, ec);
    }
