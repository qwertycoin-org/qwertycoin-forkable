...
#include <Crypto/GroestlTables.h>

#define P_TYPE 0
#define Q_TYPE 1

const uint8_t shiftValues[2][8] = { { 0, 1, 2, 3, 4, 5, 6, 7 }, { 1, 3, 5, 7, 0, 2, 4, 6 } };

const uint8_t indicesCyclic[15] = { 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6 };
...
{
    uint32_t tempV1, tempV2, tempUpperValue, tempLowerValue, temp;
    ...
    x32[12] ^= 0x00000060 ^ r;
    x32[14] ^= 0x00000070 ^ r;
    COLUMN(x, y, 0, 0, 2, 4, 6, 9, 11, 13, 15, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 2, 2, 4, 6, 8, 11, 13, 15, 1, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 4, 4, 6, 8, 10, 13, 15, 1, 3, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 6, 6, 8, 10, 12, 15, 1, 3, 5, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 8, 8, 10, 12, 14, 1, 3, 5, 7, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 10, 10, 12, 14, 0, 3, 5, 7, 9, tempV1, tempV2, tempUpperValue, tempLowerValue, temp);
    COLUMN(x, y, 12, 12, 14, 0, 2, 5, 7, 9, 11, tempV1, tempV2, tempUpperValue, tempLowerValue, temp);
    COLUMN(x, y, 14, 14, 0, 2, 4, 7, 9, 11, 13, tempV1, tempV2, tempUpperValue, tempLowerValue, temp);
}
...
static void rnd512Q(uint8_t *x, uint32_t *y, uint32_t r)
{
    ...
    x32[14] = ~x32[14];
    x32[15] ^= 0x8fffffff ^ r;
    COLUMN(x, y, 0, 2, 6, 10, 14, 1, 5, 9, 13, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 2, 4, 8, 12, 0, 3, 7, 11, 15, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 4, 6, 10, 14, 2, 5, 9, 13, 1, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 6, 8, 12, 0, 4, 7, 11, 15, 3, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 8, 10, 14, 2, 6, 9, 13, 1, 5, tempV1, tempV2, tempUpperValue, tempLowerValue,
           temp);
    COLUMN(x, y, 10, 12, 0, 4, 8, 11, 15, 3, 7, tempV1, tempV2, tempUpperValue, tempLowerValue, temp);
    COLUMN(x, y, 12, 14, 2, 6, 10, 13, 1, 5, 9, tempV1, tempV2, tempUpperValue, tempLowerValue, temp);
    COLUMN(x, y, 14, 0, 4, 8, 12, 15, 3, 7, 11, tempV1, tempV2, tempUpperValue, tempLowerValue, temp);
}
...
/* update state with dataBitLength bits of input */
static void update(HashState *ctx, const BitSequence *input, DataLengthGroestl dataBitLength)
{
    int index = 0;
    int msglen = (int)(dataBitLength / 8);
    int rem = (int)(dataBitLength % 8);

    /* if the buffer contains data that has not yet been digested, first
       add data to buffer until full */
    if (ctx->bufPtr) {
        while (ctx->bufPtr < SIZE512 && index < msglen) {
            ctx->buffer[(int)ctx->bufPtr++] = input[index++];
        }
...
#define BILB ctx->bitsInLastByte
...
static void final(HashState *ctx, BitSequence *output)
{
    ...
    if (BILB) {
        ctx->buffer[(int)ctx->bufPtr - 1] &= ((1 << BILB) - 1) << (8 - BILB);
        ctx->buffer[(int)ctx->bufPtr - 1] ^= 0x1 << (7 - BILB);
        BILB = 0;
    } else {
        ctx->buffer[(int)ctx->bufPtr++] = 0x80;
    }
    ...
    while (ctx->bufPtr > SIZE512 - (int)sizeof(uint32_t)) {
        ctx->buffer[(int)--ctx->bufPtr] = (uint8_t)ctx->blockCounter1;
        ctx->blockCounter1 >>= 8;
    }

    while (ctx->bufPtr > SIZE512 - LENGTHFIELDLEN) {
        ctx->buffer[(int)--ctx->bufPtr] = (uint8_t)ctx->blockCounter2;
        ctx->blockCounter2 >>= 8;
    }
...
/* Hash bit sequence */
void groestl(const BitSequence *data, DataLengthGroestl dataBitLength, BitSequence *hashval)
{
    ...
    /* process message */
    update(&context, data, dataBitLength);
