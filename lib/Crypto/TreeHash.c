...
void treeHash(const char (*hashes)[HASH_SIZE],
              size_t count,
              char *root_hash)
{
    assert(count > 0);
    if (count == 1) {
        ...
        cnFastHash(hashes, 2 * HASH_SIZE, root_hash);
    } else {
        size_t i, j;
        size_t cnt = count - 1;
        char(*ints)[HASH_SIZE];
        ...
        for (i = 2 * cnt - count, j = 2 * cnt - count; j < cnt; i += 2, ++j) {
            cnFastHash(hashes[i], 2 * HASH_SIZE, ints[j]);
        }
        assert(i == count);
        while (cnt > 2) {
            cnt >>= 1;
            for (i = 0, j = 0; j < cnt; i += 2, ++j) {
                cnFastHash(ints[i], 2 * HASH_SIZE, ints[j]);
            }
        }
        cnFastHash(ints[0], 2 * HASH_SIZE, root_hash);
    }
}
...
void treeBranch(const char (*hashes)[HASH_SIZE],
                size_t count,
    ...
    if (depth == 0) {
        ...
        while (depth > 0) {
            ...
            if (from_leaf) {
                ...
                cnFastHash(buffer, 2 * HASH_SIZE, leaf_path);
            }
            memcpy(branch_path, branch[depth], HASH_SIZE);
        }

        cnFastHash(buffer, 2 * HASH_SIZE, root_hash);
    }
}
