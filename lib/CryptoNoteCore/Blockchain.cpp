...
bool serialize(std::vector<std::pair<Blockchain::TransactionIndex, uint16_t>> &value,
               Common::StringView name, CryptoNote::ISerializer &s)
{
...
bool Blockchain::init(const std::string &config_folder, bool load_existing)
{
    ...
    if (m_blocks.empty()) {
        ...
        Crypto::Hash firstBlockHash = getBlockHash(m_blocks[0].bl);
...
void Blockchain::rebuildCache()
{
    ...
    for (uint32_t b = 0; b < m_blocks.size(); ++b) {
        ...
        Crypto::Hash blockHash = getBlockHash(block.bl);
...
std::vector<Crypto::Hash> Blockchain::doBuildSparseChain(const Crypto::Hash &startBlockId) const
{
    ...
    if (m_blockIndex.hasBlock(startBlockId)) {
        ...
        std::vector<Crypto::Hash> sparseMainChain =
...
Blockchain::findBlockchainSupplement(const std::vector<Crypto::Hash> &remoteBlockIds,
                                ...
                                uint32_t *pmax_related_block_height)
{
    ...
    struct outputs_visitor {
        outputs_visitor(std::vector<const Crypto::PublicKey *> &results_collector, Blockchain &bch,
                        ILogger &logger)
                           ...
                           block_verification_context &bvc)
{
    ...
    Crypto::Hash blockHash = getBlockHash(blockData);
