        ...
        if (loggersList.isArray()) {
            ...
            for (size_t i = 0; i < countOfLoggers; ++i) {
                ...
                if (loggerConfiguration.contains("disabledCategories")) {
                    ...
                    for (size_t ii = 0; ii < countOfCategories; ++ii) {
                        auto categoryVal = disabledCategoriesVal[ii];
                        if (categoryVal.isString()) {
...
void LoggerManager::operator()(const std::string &category,
                               Level level,
