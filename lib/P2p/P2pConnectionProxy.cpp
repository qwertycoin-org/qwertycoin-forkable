...
void P2pConnectionProxy::write(const P2pMessage &message)
{
    if (message.type == COMMAND_HANDSHAKE::ID) {
        ...
        m_context.writeMessage(
            P2pContext::Message(P2pMessage(message), P2pContext::Message::NOTIFY)
        );
    }
}
...
void P2pConnectionProxy::writeHandshake(const P2pMessage &message)
{
    ...
    if (m_context.isIncoming()) {
        ...
        m_context.writeMessage(
            makeReply(COMMAND_HANDSHAKE::ID,
                      LevinProtocol::encode(res),
...
void P2pConnectionProxy::handleHandshakeRequest(const LevinProtocol::Command &cmd)
{
    ...
    m_readQueue.push(P2pMessage{cmd.command, LevinProtocol::encode(req.payload_data) });
}

void P2pConnectionProxy::handleHandshakeResponse(const LevinProtocol::Command &cmd,
                                                 P2pMessage &message)
{
    if (m_context.isIncoming()) {
...
void P2pConnectionProxy::handleTimedSync(const LevinProtocol::Command &cmd)
{
    if (cmd.isResponse) {
        ...
        m_context.writeMessage(
            makeReply(COMMAND_TIMED_SYNC::ID,
                      LevinProtocol::encode(res),
                      LEVIN_PROTOCOL_RETCODE_SUCCESS)
        );
    }
