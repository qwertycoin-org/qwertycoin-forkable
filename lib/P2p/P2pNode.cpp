...
void doWithTimeoutAndThrow(System::Dispatcher &dispatcher,
...
void P2pNode::makeExpectedConnectionsCount(const PeerlistManager::Peerlist &peerlist,
...
bool P2pNode::fetchPeerList(ContextPtr connection)
{
    try {
        ...
        if (response.node_data.version < QwertyNote::P2P_MINIMUM_VERSION) {
            logger(ERROR)
                << *connection
                << "COMMAND_HANDSHAKE Failed, peer is wrong version: "
                << std::to_string(response.node_data.version);
            return false;
        } else if ((response.node_data.version - QwertyNote::P2P_CURRENT_VERSION) >= QwertyNote::P2P_UPGRADE_WINDOW) {
            logger(WARNING)
                << *connection
...
basic_node_data P2pNode::getNodeData() const
{
    ...
    nodeData.version = QwertyNote::P2P_CURRENT_VERSION;
...
{
    if (ctx.getPeerId() == m_myPeerId
        || !m_peerlist.is_ip_allowed(ctx.getRemoteAddress().ip)
        || ctx.getPeerPort() == 0) {
        return;
    }
    ...
    try {
        ...
        doWithTimeoutAndThrow(m_dispatcher, m_cfg.getHandshakeTimeout(), [&]() {
            ...
            if (response.status == PING_OK_RESPONSE_STATUS_TEXT
                && response.peer_id == ctx.getPeerId()) {
                PeerlistEntry entry;
...
void P2pNode::handleNodeData(const basic_node_data &node, P2pContext &context)
{
    ...
    if (node.version < QwertyNote::P2P_MINIMUM_VERSION) {
        std::ostringstream msg;
        msg << context
            << "COMMAND_HANDSHAKE Failed, peer is wrong version! ("
