...
void MemoryMappedFile::create(const std::string &path,
                              uint64_t size,
                              bool overwrite, std::error_code &ec)
{
    ...
    Tools::QScopeExit failExitHandler([this, &ec] {
    ...
    m_file = ::open(path.c_str(),
                    O_RDWR | O_CREAT | (overwrite ? O_TRUNC : O_EXCL), S_IRUSR | S_IWUSR);
    if (m_file == -1) {
        return;
    }
...
void MemoryMappedFile::flush(uint8_t *data, uint64_t size, std::error_code &ec)
{
    ...
    auto pageSize = static_cast<uintptr_t>(sysconf(_SC_PAGESIZE));
    auto dataAddr = reinterpret_cast<uintptr_t>(data);
    uintptr_t pageOffset = (dataAddr / pageSize) * pageSize;

    int result = ::msync(reinterpret_cast<void *>(pageOffset),
                         static_cast<size_t>(dataAddr % pageSize + size),
                         MS_SYNC);
    if (result == 0) {
        result = ::fsync(m_file);
        if (result == 0) {
