...
bool PaymentIdIndex::find(const Crypto::FHash&paymentId, std::vector<Crypto::FHash>&transactionHashes)
{
    if (!enabled) {
...
std::vector<Crypto::FHash> PaymentIdIndex::find(const Crypto::FHash &paymentId)
{
    if (!enabled) {
        throw std::runtime_error("Payment id index disabled.");
    }
...
bool TimestampBlocksIndex::add(uint64_t timestamp, const Crypto::FHash &hash)
{
    if (!enabled) {
        return false;
    }
...
bool TimestampBlocksIndex::remove(uint64_t timestamp, const Crypto::FHash &hash)
{
    if (!enabled) {
        return false;
    }
...
bool TimestampBlocksIndex::find(
...
bool TimestampTransactionsIndex::add(uint64_t timestamp, const Crypto::FHash &hash)
{
    if (!enabled) {
        return false;
    }
...
bool TimestampTransactionsIndex::remove(uint64_t timestamp, const Crypto::FHash &hash)
{
    if (!enabled) {
        return false;
    }
...
bool TimestampTransactionsIndex::find(
    uint64_t timestampBegin,
    uint64_t timestampEnd,
...
bool OrphanBlocksIndex::find(uint32_t height, std::vector<Crypto::FHash> &blockHashes)
{
    if (!enabled) {
