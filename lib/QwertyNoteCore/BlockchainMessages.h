...
{
public:
    explicit NewBlockMessage(const Crypto::FHash &hash);
    NewBlockMessage() = default;

    void get(Crypto::FHash &hash) const;
...
{
public:
    explicit NewAlternativeBlockMessage(const Crypto::FHash &hash);
...
{
public:
    explicit ChainSwitchMessage(std::vector<Crypto::FHash> &&hashes);
    ChainSwitchMessage(const ChainSwitchMessage &other);

    void get(std::vector<Crypto::FHash> &hashes) const;
...
public:
    ...
    bool getNewBlockHash(Crypto::FHash &hash) const;
    bool getNewAlternativeBlockHash(Crypto::FHash &hash) const;
    bool getChainSwitch(std::vector<Crypto::FHash> &hashes) const;
