...
bool Checkpoints::add_checkpoint(uint32_t height, const std::string &hash_str)
{
    ...
    if (!Common::podFromHex(hash_str, h)) {
        logger(WARNING) << "Wrong FHash in checkpoint for height " << height;
        return false;
    }
...
bool Checkpoints::check_block(uint32_t  height, const Crypto::FHash &h, bool &is_a_checkpoint) const
{
...
bool Checkpoints::check_block(uint32_t  height, const Crypto::FHash &h) const
{
    bool ignored;
    ...
    uint32_t  block_height) const
{
    ...
    if (block_height < blockchain_height - parameters::MINED_MONEY_UNLOCK_WINDOW
...
{
    std::string domain(QwertyNote::DNS_CHECKPOINTS_HOST);
