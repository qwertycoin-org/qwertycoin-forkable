...
bool Currency::init()
{
    ...
    if (!getBlockHash(m_genesisBlock, m_genesisBlockHash)) {
        logger(ERROR, BRIGHT_RED) << "Failed to get genesis block FHash";
        return false;
    }
...
{
    return QwertyNote::parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE;
}
                              ...
                              uint64_t blockTarget) const
{
    ...
    if (height > QwertyNote::parameters::UPGRADE_HEIGHT_V1 && difficultyTarget() != 0) {
    ...
    if (alreadyGeneratedCoins + QwertyNote::parameters::TAIL_EMISSION_REWARD >= m_moneySupply
        || baseReward < QwertyNote::parameters::TAIL_EMISSION_REWARD) {
        baseReward = QwertyNote::parameters::TAIL_EMISSION_REWARD;
    }
...
bool Currency::getGovernanceAddressAndKey(AccountKeys &governanceKeys) const
{
    ...
    Crypto::FSecretKey governanceViewSecretKey{};
                                         ...
                                         lazy_stat_callback_type &lazyStatCallbackType) const
{
    if (isTestnet() || timestamps.empty()) {
        return QwertyNote::parameters::DEFAULT_DIFFICULTY;
    }

    // check if we use special scenario with some fixed diff
    if (QwertyNote::parameters::FIXED_DIFFICULTY > 0) {
        logger (WARNING) << "Fixed difficulty is used: " << QwertyNote::parameters::FIXED_DIFFICULTY;
        return QwertyNote::parameters::FIXED_DIFFICULTY;
    }
    ...
    if (nextBlockTime > lastTimestamp + QwertyNote::parameters::CRYPTONOTE_CLIF_THRESHOLD) {
    ...
    difficulty_type nextEPoWDiff = QwertyNote::parameters::DEFAULT_DIFFICULTY;
    difficulty_type min_difficulty = QwertyNote::parameters::DEFAULT_DIFFICULTY;
    ...
    if (height < QwertyNote::parameters::UPGRADE_HEIGHT_V1 + diffWindow) {
        return nextEPoWDiff;
    }
    ...
    uint64_t difficulty_target = QwertyNote::parameters::DIFFICULTY_TARGET;
    ...
    // process uData with "invalid" solvetimes
                                            ...
                                            lazy_stat_callback_type &lazy_stat_cb) const
{
    ...
    if (new_diff > QwertyNote::parameters::DEFAULT_DIFFICULTY) {
        uint64_t correction_interval =
                currentSolveTime - QwertyNote::parameters::CRYPTONOTE_CLIF_THRESHOLD;
        // below equation shall return quotient of the division.
        int decrease_counter =
                ((int)correction_interval / (int)QwertyNote::parameters::DIFFICULTY_TARGET) + 1;
        int round_counter = 1;
        ...
        new_diff = std::max(new_diff, difficulty_type(QwertyNote::parameters::DEFAULT_DIFFICULTY));
    }
                                                 ...
                                                 size_t mixinCount) const
{
    ...
    const size_t SIGNATURE_SIZE = sizeof(Crypto::FSignature);
    const size_t EXTRA_TAG_SIZE = sizeof(uint8_t);
    const size_t INPUT_TAG_SIZE = sizeof(uint8_t);
    const size_t OUTPUT_TAG_SIZE = sizeof(uint8_t);
    const size_t PUBLIC_KEY_SIZE = sizeof(Crypto::FPublicKey);
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger &log) : m_currency(log)
{
    ...
    publicAddressBase58Prefix(parameters::ADDRESS_BASE58_PREFIX);
    minedMoneyUnlockWindow(parameters::TX_SPENDABLE_AGE);
    transactionSpendableAge(parameters::TX_SPENDABLE_AGE);
    safeTransactionSpendableAge(parameters::SAFE_TX_SPENDABLE_AGE);
    ...
    cryptonoteCoinVersion(parameters::COIN_VERSION);
    ...
    numberOfDecimalPlaces(parameters::DISPLAY_DECIMAL_POINT);
