...
bool miner::request_block_template()
{
    ...
    if(!m_extra_messages.empty() && m_config.current_extra_message_index < m_extra_messages.size()){
...
void miner::merge_hr()
{
    if(m_last_hr_merge_time && is_mining()) {
        ...
        if (m_do_print_hashrate) {
            uint64_t total_hr = std::accumulate(
                m_last_hash_rates.begin(),
                m_last_hash_rates.end(),
    ...
    if (!config.extraMessages.empty()) {
        ...
        if (!Common::loadFileToString(config.extraMessages, buff)) {
            logger(ERROR, BRIGHT_RED)
                << "Failed to load file with extra messages: "
                << config.extraMessages;
            return false;
        }
        ...
        std::string filebuf;
        if (Common::loadFileToString(
                m_config_folder_path + "/"
                + QwertyNote::parameters::MINER_CONFIG_FILE_NAME, filebuf)
            ) {
        ...
        logger(INFO)
            << "Loaded "
            << m_extra_messages.size()
            << " extra messages, current index "
        ...
        if (!m_stop && check_hash(h, local_diff)) {
            ...
            if(!m_handler.handle_block_found(b)) {
                ...
                Common::saveStringToFile(
                    m_config_folder_path
                    + "/"
                    + QwertyNote::parameters::MINER_CONFIG_FILE_NAME, storeToJson(m_config)
                );
            }
        }
