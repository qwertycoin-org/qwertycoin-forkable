...
public:
    ...
    public:
        const_iterator(
            SwappedMap *swappedMap,
            typename std::unordered_map<Key, Descriptor>::const_iterator descriptorsIterator)
            : m_swappedMap(swappedMap),
              m_descriptorsIterator(descriptorsIterator)
        {
        }
...
{
    std::cout
        << "SwappedMap cache hits: " << m_cacheHits
...
void SwappedMap<Key, T>::erase(const_iterator iterator)
{
    ...
    auto descriptorsIterator = iterator.innerIterator();
    m_indexesFile.seekp(
        sizeof(uint64_t)
        + (sizeof(bool)
        + sizeof(Key)
        + sizeof(uint32_t)) * descriptorsIterator->second.index
    );
    bool valid = false;
    ...
    const std::pair<const Key, T> &value)
{
...

    {
        ...
        m_indexesFile.seekp(
            ...
            + (sizeof(bool)
            + sizeof(Key)
            + sizeof(uint32_t)) * descriptorsCounter
        );
        bool valid = true;
        m_indexesFile.write(reinterpret_cast<char*>(&valid), sizeof valid);
        if (!m_indexesFile) {
        ...
        auto itemSize = static_cast<uint32_t>(itemsFileSize - m_itemsFileSize);
...
std::pair<const Key, T> *SwappedMap<Key, T>::prepare(const Key& key)
{
    ...
    auto itemInsert = m_items.insert(std::make_pair(key, T()));
...
const std::pair<const Key, T> *SwappedMap<Key, T>::load(const Key& key, uint64_t offset)
{
    ...
    auto descriptorsIterator = m_descriptors.find(key);
