...
FPublicKey getTransactionPublicKeyFromExtra(const std::vector<uint8_t> &tx_extra)
{
    ...
    if (!findTransactionExtraFieldByType(tx_extra_fields, pub_key_field)) {
        return boost::value_initialized<FPublicKey>();
    }

    return pub_key_field.publicKey;
}

bool addTransactionPublicKeyToExtra(std::vector<uint8_t> &tx_extra, const FPublicKey &tx_pub_key)
{
    tx_extra.resize(tx_extra.size() + 1 + sizeof(FPublicKey));
    tx_extra[tx_extra.size() - 1 - sizeof(FPublicKey)] = TX_EXTRA_TAG_PUBKEY;

    *reinterpret_cast<FPublicKey*>(&tx_extra[tx_extra.size() - sizeof(FPublicKey)]) = tx_pub_key;
...
std::vector<std::string> getMessagesFromExtra(
    ...
    const Crypto::FPublicKey &txkey,
    Crypto::FSecretKey *recepient_secret_key)
{
    std::vector<TransactionExtraField> tx_extra_fields;
...
void setPaymentIdToTransactionExtraNonce(std::vector<uint8_t> &extra_nonce, const FHash &payment_id)
{
    extra_nonce.clear();
...
bool getPaymentIdFromTransactionExtraNonce(const std::vector<uint8_t> &extra_nonce, FHash &payment_id)
{
    if (sizeof(FHash) + 1 != extra_nonce.size()) {
        return false;
    }
    ...
    payment_id = *reinterpret_cast<const FHash *>(extra_nonce.data() + 1);

    return true;
}

bool parsePaymentId(const std::string &paymentIdString, FHash &paymentId)
{
...
bool createTxExtraWithPaymentId(const std::string &paymentIdString, std::vector<uint8_t> &extra)
{
    ...
    QwertyNote::setPaymentIdToTransactionExtraNonce(extraNonce, paymentIdBin);
    if (!QwertyNote::addExtraNonceToTransactionExtra(extra, extraNonce)) {
        return false;
    }
...
bool getPaymentIdFromTxExtra(const std::vector<uint8_t> &extra, FHash &paymentId)
{
    std::vector<TransactionExtraField> tx_extra_fields;
    if (!parseTransactionExtra(extra, tx_extra_fields)) {
        return false;
    ...
    const KeyPair &txkey)
{
    ...
    if (recipient) {
        ...
        key_data.magic2 = 0;
        FHash h = cnFastHash(&key_data, sizeof(message_key_data));
    ...
    sData.assign(buf.get(), mlen);

    return true;
}

bool tx_extra_message::decrypt(
    size_t index, const Crypto::FPublicKey &txkey,
    Crypto::FSecretKey *recepient_secret_key,
    std::string &message) const
{
    size_t mlen = sData.size();
    ...
    if (recepient_secret_key != nullptr) {
        ...
        uint64_t nonce = SWAP64LE(index);
        uint8_t *nonce8 = reinterpret_cast<uint8_t *>(&nonce);
        chacha(10, sData.data(), mlen, reinterpret_cast<uint8_t *>(&h), nonce8, ptr.get());
        buf = ptr.get();
    } else {
        buf = sData.data();
    }
    ...
    const KeyPair &txkey)
{
    ...
    if (recipient) {
        ...
        key_data.magic1 = 0x80;
        key_data.magic2 = 0;
        FHash h = cnFastHash(&key_data, sizeof(sender_key_data));
        uint64_t nonce = SWAP64LE(index);
        uint8_t *nonce8 = reinterpret_cast<uint8_t *>(&nonce);
        chacha(10, buf.get(), mlen, reinterpret_cast<uint8_t *>(&h), nonce8, buf.get());
    }
