...
public:
    ...
    const std::vector<Crypto::FHash> &getTransactions() const
    {
        return m_txHashes;
    }
...
private:
    std::unordered_set<Crypto::FKeyImage> m_keyImages;
    std::set<std::pair<uint64_t, uint64_t>> m_usedOutputs;
    std::vector<Crypto::FHash> m_txHashes;
};

using QwertyNote::BlockInfo;

std::unordered_set<Crypto::FHash> m_validated_transactions;

//---------------------------------------------------------------------------------
tx_memory_pool::tx_memory_pool(
    const QwertyNote::Currency &currency,
                               QwertyNote::ITransactionValidator &validator,
                               QwertyNote::ICore &core, QwertyNote::ITimeProvider &timeProvider,
    Logging::ILogger &log,
...
std::list<QwertyNote::tx_memory_pool::TransactionDetails> tx_memory_pool::getMemoryPool() const
{
    std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
...
{
    std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
    std::unordered_set<Crypto::FHash> ready_tx_ids;
    ...
    std::unordered_set<Crypto::FHash> known_set(known_tx_ids.begin(), known_tx_ids.end());
...
bool tx_memory_pool::on_blockchain_inc(uint64_t new_block_height, const Crypto::FHash& top_block_id)
{
    std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
...
bool tx_memory_pool::on_blockchain_dec(uint64_t new_block_height, const Crypto::FHash& top_block_id)
{
    std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
    if (!m_validated_transactions.empty()) {
...
bool tx_memory_pool::have_tx(const Crypto::FHash &id) const
{
    std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
...
bool tx_memory_pool::deinit()
{
    if (!Tools::createDirectoriesIfNecessary(m_config_folder)) {
      logger(INFO) << "Failed to create uData directory: " << m_config_folder;
      return false;
    }
...
void serialize(QwertyNote::tx_memory_pool::TransactionDetails &td, ISerializer &s)
{
    s(td.id, "id");
