...
static_assert(QwertyNote::UpgradeDetectorBase::UNDEF_HEIGHT == UINT32_C(0xFFFFFFFF),
    ...
    bool init()
    {
        ...
        if (upgradeHeight == UNDEF_HEIGHT) {
            if (m_blockchain.empty()) {
                ...
                uint32_t uh = it - m_blockchain.begin();
                m_votingCompleteHeight = findVotingCompleteHeight(uh);
...
private:
    ...
    bool isVotingComplete(uint32_t height)
    {
        ...
        auto voteCounterX = voteCounter * 100;

        return m_currency.upgradeVotingThreshold()*m_currency.upgradeVotingWindow()<=voteCounterX;
    }
