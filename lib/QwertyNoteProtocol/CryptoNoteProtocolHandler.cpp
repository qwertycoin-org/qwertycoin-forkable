...
bool post_notify(IP2pEndpoint &p2p,
...
template<class t_parametr>
void relay_post_notify(IP2pEndpoint &p2p,
                       typename t_parametr::request &arg,
...
void CryptoNoteProtocolHandler::onConnectionClosed(CryptoNoteConnectionContext &context)
{
    ...
    if (updated) {
        ...
        m_observerManager.notify(&IQwertyNoteProtocolObserver::lastKnownBlockHeightUpdated,
                                 m_observedHeight);
    }

    if (context.m_state != CryptoNoteConnectionContext::state_befor_handshake) {
        m_peersCount--;
        m_observerManager.notify(&IQwertyNoteProtocolObserver::peerCountUpdated,
                                 m_peersCount.load());
    }
}
    ...
    bool is_initial)
{
    ...
    if (context.m_state == CryptoNoteConnectionContext::state_synchronizing) {
        ...
        logger(
            ...
            << "Sync uData returned unknown top block: " << get_current_blockchain_height()
        ...
        // to let send request uData after response
...
int CryptoNoteProtocolHandler::handle_notify_new_block(int command,
                                                       ...
                                                       CryptoNoteConnectionContext &context)
{
    ...
    for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
        QwertyNote::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
        ...
        if (tvc.m_verification_failed) {
            logger(Logging::INFO)
                ...
                << "transaction verification failed, dropping connection";
    ...
    CryptoNoteConnectionContext &context)
{
    ...
    for (auto tx_blob_it = arg.txs.begin(); tx_blob_it != arg.txs.end();) {
        ...
        logger(DEBUGGING)
            << "transaction "
            << transactionHash
            << " came in NOTIFY_NEW_TRANSACTIONS";
    ...
    CryptoNoteConnectionContext &context)
{
    ...
    //
    // This is a partial fix. Thanks and credit given to QwertyNote author
    ...
    for (const BlockCompleteEntry & block_entry : arg.blocks) {
        ...
        if (count == 2) {
            if (m_core.have_block(getBlockHash(b))) {
                ...
                logger(Logging::DEBUGGING) << context << "Connection set to idle mState.";
                return 1;
            }
        }
                                              ...
                                              const std::vector<BlockCompleteEntry> &blocks)
{
    for (const BlockCompleteEntry &block_entry : blocks) {
        ...
        if (bvc.m_verification_failed) {
            ...
            logger(Logging::DEBUGGING)
                << context
                << "Block already exists, switching to idle mState";
            context.m_state = CryptoNoteConnectionContext::state_idle;
            context.m_needed_objects.clear();
            context.m_requested_objects.clear();
            return 1;
...
int CryptoNoteProtocolHandler::handle_request_chain(int command,
                                                    NOTIFY_REQUEST_CHAIN::request &arg,
                                                    CryptoNoteConnectionContext &context)
{
    logger(Logging::TRACE)
        << context
        << "NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << arg.block_ids.size();
...
bool CryptoNoteProtocolHandler::on_connection_synchronized()
{
    ...
    if (m_synchronized.compare_exchange_strong(val_expected, true)) {
        logger(Logging::INFO)
        << ENDL ;
        logger(INFO, BRIGHT_MAGENTA) << "===[ " + std::string(QwertyNote::CRYPTONOTE_NAME) + " Tip! ]=============================" << ENDL ;
        logger(INFO, WHITE) << " Always exit " + WalletConfig::daemonName + " and " + WalletConfig::walletName + " with the \"exit\" command to preserve your chain and wallet uData." << ENDL ;
        ...
        m_observerManager.notify(&IQwertyNoteProtocolObserver::blockchainSynchronized, height);
    }
    ...
    CryptoNoteConnectionContext &context)
{
    ...
    context.m_last_response_height = arg.start_height
...
void CryptoNoteProtocolHandler::recalculateMaxObservedHeight(
    const CryptoNoteConnectionContext &context)
{
    //should be locked outside
    uint32_t peerHeight = 0;
    m_p2p->for_each_connection([&peerHeight, &context](const CryptoNoteConnectionContext &ctx,
                                                       PeerIdType peerId) {
...
bool CryptoNoteProtocolHandler::addObserver(IQwertyNoteProtocolObserver *observer)
{
    return m_observerManager.add(observer);
}

bool CryptoNoteProtocolHandler::removeObserver(IQwertyNoteProtocolObserver *observer)
{
