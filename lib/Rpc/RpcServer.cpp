...
std::unordered_map<std::string, RpcServer::RpcHandler<RpcServer::HandlerFunction>>
        RpcServer::s_handlers = {
            // binary handlers
            { "/getblocks.bin",
              { binMethod<COMMAND_RPC_GET_BLOCKS_FAST>(&RpcServer::onGetBlocks), false } },
            { "/queryblocks.bin",
              { binMethod<COMMAND_RPC_QUERY_BLOCKS>(&RpcServer::onQueryBlocks), false } },
            { "/queryblockslite.bin",
              { binMethod<COMMAND_RPC_QUERY_BLOCKS_LITE>(&RpcServer::onQueryBlocksLite), false } },
            { "/get_o_indexes.bin",
              { binMethod<COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES>(&RpcServer::onGetIndexes),
                false } },
            { "/getrandom_outs.bin",
              { binMethod<COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS>(&RpcServer::onGetRandomOuts),
                false } },
            { "/get_pool_changes.bin",
              { binMethod<COMMAND_RPC_GET_POOL_CHANGES>(&RpcServer::onGetPoolChanges), false } },
            { "/get_pool_changes_lite.bin",
              { binMethod<COMMAND_RPC_GET_POOL_CHANGES_LITE>(&RpcServer::onGetPoolChangesLite),
                false } },

            // http handlers
            { "/", { httpMethod<COMMAND_HTTP>(&RpcServer::onGetIndex), true } },
            { "/supply", { httpMethod<COMMAND_HTTP>(&RpcServer::onGetSupply), false } },
            { "/paymentid", { httpMethod<COMMAND_HTTP>(&RpcServer::onGetPaymentId), false } },

            // json get handlers
            { "/getinfo", { jsonMethod<COMMAND_RPC_GET_INFO>(&RpcServer::onGetInfo), true } },
            { "/getversion",
              { jsonMethod<COMMAND_RPC_GET_VERSION>(&RpcServer::onGetVersion), true } },
            { "/gethardwareinfo",
              { jsonMethod<COMMAND_RPC_GET_HARDWARE_INFO>(&RpcServer::onGetHardwareInfo), true } },
            { "/getheight", { jsonMethod<COMMAND_RPC_GET_HEIGHT>(&RpcServer::onGetHeight), true } },
            { "/feeaddress",
              { jsonMethod<COMMAND_RPC_GET_FEE_ADDRESS>(&RpcServer::onGetFeeAddress), true } },
            { "/get_mempool",
              { jsonMethod<COMMAND_RPC_GET_POOL>(&RpcServer::onTransactionsPoolJson), false } },
            { "/get_mempool_detailed",
              { jsonMethod<COMMAND_RPC_GET_MEM_POOL>(&RpcServer::onMempoolJson), false } },
            { "/gettransactionspool",
              { jsonMethod<COMMAND_RPC_GET_TRANSACTIONS_POOL_SHORT>(
                        &RpcServer::onGetTransactionsPoolShort),
                true } },
            { "/gettransactionsinpool",
              { jsonMethod<COMMAND_RPC_GET_TRANSACTIONS_POOL>(&RpcServer::onGetTransactionsPool),
                true } },
            { "/getrawtransactionspool",
              { jsonMethod<COMMAND_RPC_GET_RAW_TRANSACTIONS_FROM_POOL>(
                        &RpcServer::onGetRawTransactionPool),
                true } },

            // json post handlers
            { "/gettransactions",
              { jsonMethod<COMMAND_RPC_GET_TRANSACTIONS>(&RpcServer::onGetTransactions), false } },
            { "/sendrawtransaction",
              { jsonMethod<COMMAND_RPC_SEND_RAW_TX>(&RpcServer::onSendRawTx), false } },
            { "/getblocks",
                    { jsonMethod<COMMAND_RPC_GET_BLOCKS_FAST>(&RpcServer::onGetBlocks), false } },
            { "/queryblocks",
              { jsonMethod<COMMAND_RPC_QUERY_BLOCKS>(&RpcServer::onQueryBlocks), false } },
            { "/queryblockslite",
              { jsonMethod<COMMAND_RPC_QUERY_BLOCKS_LITE>(&RpcServer::onQueryBlocksLite), false } },
            { "/queryblocksdetailed",
              { jsonMethod<COMMAND_RPC_QUERY_BLOCKS_DETAILED>(&RpcServer::onQueryBlocksDetailed),
                false } },

            { "/get_o_indexes",
              { jsonMethod<COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES>(&RpcServer::onGetIndexes),
                false } },
            { "/getrandom_outs",
              { jsonMethod<COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS>(&RpcServer::onGetRandomOuts),
                false } },
            { "/get_pool_changes",
              { jsonMethod<COMMAND_RPC_GET_POOL_CHANGES>(&RpcServer::onGetPoolChanges), false } },
            { "/get_pool_changes_lite",
              { jsonMethod<COMMAND_RPC_GET_POOL_CHANGES_LITE>(&RpcServer::onGetPoolChangesLite),
                false } },
            ...
            { "/get_raw_transactions_by_heights",
              { jsonMethod<COMMAND_RPC_GET_RAW_TRANSACTIONS_BY_HEIGHTS>(
                        &RpcServer::onGetRawTransactionsByHeights),
                false } },
            ...
            { "/start_mining",
              { jsonMethod<COMMAND_RPC_START_MINING>(&RpcServer::onStartMining), false } },
            { "/stop_mining",
              { jsonMethod<COMMAND_RPC_STOP_MINING>(&RpcServer::onStopMining), false } },
            { "/stop_daemon",
              { jsonMethod<COMMAND_RPC_STOP_DAEMON>(&RpcServer::onStopDaemon), true } },
            { "/get_difficulty_stat",
              { jsonMethod<COMMAND_RPC_GET_DIFFICULTY_STAT>(&RpcServer::onGetDifficultyStat),
                false } },
            { "/getpeers",
              { jsonMethod<COMMAND_RPC_GET_PEER_LIST>(&RpcServer::onGetPeerList), true } },
            { "/peers",
              { jsonMethod<COMMAND_RPC_GET_PEER_LIST>(&RpcServer::onGetPeerList), true } },
        ...
        if (it == s_handlers.end()) {
            if (Common::startsWith(url, "/api/")) {
                ...
                if (Common::startsWith(url, block_height_method)) {
                    ...
                    FBlock blk;
                    std::vector<Crypto::FHash> vh;
                    uint32_t upperBorder = std::min(tempInt, m_core.getCurrentBlockchainHeight());
                    Crypto::FHash blockHash = m_core.getBlockIdByHeight(upperBorder);
                    ...
                    if (blk.sBaseTransaction.vInputs.front().type() != typeid(FBaseInput)) {
                        throw JsonRpc::JsonRpcError {
                            CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                    ...
                    for (Crypto::FHash &bTxs : blk.vTransactionHashes) {
                        vh.push_back(bTxs);
                    }

                    vh.push_back(getObjectHash(blk.sBaseTransaction));
                    ...
                    FTransactionDetails transactionsDetails;

                    bool r = blockchainExplorerDataBuilder.fillTransactionDetails(
                            txs.back(), transactionsDetails);
                    if (r) {
                        response.addHeader("Content-Type", "application/json");
                        response.setStatus(HttpResponse::HTTP_STATUS::STATUS_200);
                    ...
                    COMMAND_RPC_GET_MEM_POOL::request req;
                    COMMAND_RPC_GET_MEM_POOL::response rsp;
    ...
    try {
        ...
        static std::unordered_map<std::string, RpcServer::RpcHandler<JsonMemberMethod>>
                jsonRpcHandlers = {
                    ...
                    { "on_getblockhash", { makeMemberMethod(&RpcServer::onGetBlockHash), true } },
                    { "getblockhash", { makeMemberMethod(&RpcServer::onGetBlockHash), true } },
                    { "getblocktemplate",
                    ...
                    { "getblockslist", { makeMemberMethod(&RpcServer::onBlocksListJson), false } },
                    { "getaltblockslist",
                    ...
                    { "getstatsbyheights",
                      { makeMemberMethod(&RpcServer::onGetStatsByHeights), false } },
                    { "getstatsinrange",
                      { makeMemberMethod(&RpcServer::onGetStatsByHeightsRange), false } },
                    ...
                    { "check_tx_proof", { makeMemberMethod(&RpcServer::onCheckTxProof), false } },
                    { "check_reserve_proof",
...
bool RpcServer::setFeeAddress(const std::string &fee_address, const FAccountPublicAddress &fee_acc)
{
...
bool RpcServer::setViewKey(const std::string &view_key)
{
    ...
    m_view_key = *(struct Crypto::FSecretKey *)&private_view_key_hash;
...
bool RpcServer::masternodeCheckIncomingTx(const BinaryArray &tx_blob)
{
    ...
    bool isFusionTransaction = m_core.currency().isFusionTransaction(
            tx, tx_blob.size(), m_core.getCurrentBlockchainHeight() - 1);
    if (fee == 0 && isFusionTransaction) {
        logger(DEBUGGING) << "Masternode received fusion transaction, relaying with no fee check";
        return true;
    }

    QwertyNote::FTransactionPrefix transaction = *static_cast<const FTransactionPrefix *>(&tx);
                            ...
                            COMMAND_RPC_GET_BLOCKS_FAST::response &res)
{
    ...
    std::vector<Crypto::FHash> supplement = m_core.findBlockchainSupplement(
                                 ...
                                 COMMAND_RPC_GET_POOL_CHANGES::response &rsp)
{
    ...
    std::vector<QwertyNote::FTransaction> addedTransactions;
...
{
    try {
        std::vector<FBlockDetails> blockDetails;
            ...
            if (m_core.getCurrentBlockchainHeight() <= height) {
                throw JsonRpc::JsonRpcError {
                    ...
                    std::string("To big height: ") + std::to_string(height)
                            ...
                            + std::to_string(m_core.getCurrentBlockchainHeight() - 1)
                };
            }
            FHash block_hash = m_core.getBlockIdByHeight(height);
            FBlock blk;
        ...
        COMMAND_RPC_GET_BLOCKS_DETAILS_BY_HASHES::response &rsp)
{
    try {
        std::vector<FBlockDetails> blockDetails;
        for (const Crypto::FHash &hash : req.blockHashes) {
            FBlock blk;
        ...
        COMMAND_RPC_GET_BLOCK_DETAILS_BY_HEIGHT::response &rsp)
{
    try {
        ...
        FHash block_hash = m_core.getBlockIdByHeight(req.blockHeight);
        FBlock blk;
        if (!m_core.getBlockByHash(block_hash, blk)) {
            throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                          "Internal error: can't get block by height "
...
{
    try {
        std::vector<FTransactionDetails> transactionsDetails;
        ...
        COMMAND_RPC_GET_TRANSACTION_DETAILS_BY_HASH::response &rsp)
{
    try {
        ...
        FTransactionDetails transactionsDetails;
        if (!blockchainExplorerDataBuilder.fillTransactionDetails(txs.back(),
                                                                  transactionsDetails)) {
            throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                          "Internal error: can't fill transaction details." };
        }
...
bool RpcServer::onGetIndex(const COMMAND_HTTP::request &req, COMMAND_HTTP::response &res)
{
    ...
    const std::string uptime_str = std::to_string((unsigned int)floor(uptime / 60.0 / 60.0 / 24.0))
            + "d " + std::to_string((unsigned int)floor(fmod((uptime / 60.0 / 60.0), 24.0))) + "h "
    ...
    Crypto::FHash last_block_hash = m_core.getBlockIdByHeight(top_block_index);
                          ...
                          COMMAND_RPC_GET_INFO::response &res)
{
    ...
    Crypto::FHash last_block_hash =
            m_core.getBlockIdByHeight(m_core.getCurrentBlockchainHeight() - 1);
    ...
    uint32_t lastBlockHeight = boost::get<FBaseInput>(blk.sBaseTransaction.vInputs.front()).uBlockIndex;

    Crypto::FHash tmp_hash = m_core.getBlockIdByHeight(lastBlockHeight);
        ...
        COMMAND_RPC_GET_TRANSACTIONS_BY_HEIGHTS::response &res)
{
    try {
        ...
        if (req.range) {
            ...
            uint32_t upperBorder = std::min(req.heights[1], m_core.getCurrentBlockchainHeight());

            for (size_t i = 0; i < (upperBorder - req.heights[0]); i++) {
                FBlock blk;
                Crypto::FHash blockHash = m_core.getBlockIdByHeight(req.heights[0] + i);
            ...
            for (size_t i = 0; i < req.heights.size(); i++) {
                FBlock blk;
                Crypto::FHash blockHash = m_core.getBlockIdByHeight(req.heights[i]);
        ...
        std::vector<Crypto::FHash>::const_iterator vHi = vh.begin();
        ...
        for (const FTransaction &tx : txs) {
            ...
            Crypto::FHash txHash = *vHi++;
            e.tx_hash = *txHi++;

            bool r = m_core.getBlockContainingTx(txHash, blockHash, blockHeight);
            bool oR = m_core.getTxOutputsGlobalIndexes(txHash, e.output_indices);
...
bool RpcServer::onGetRawTransactionsByHeights(
        ...
        COMMAND_RPC_GET_RAW_TRANSACTIONS_BY_HEIGHTS::response &res)
{
    try {
        ...
        for (const uint32_t &height : heights) {
            ...
            Crypto::FHash blockHash = m_core.getBlockIdByHeight(height);
            FBlock blk;
            std::vector<Crypto::FHash> txsIds;
            if (!m_core.getBlockByHash(blockHash, blk)) {
                throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                              "Internal error: can't get block by height "
                                                      + std::to_string(height) + '.' };
            ...
            if (req.includeMinerTxs) {
                txsIds.reserve(blk.vTransactionHashes.size() + 1);
                txsIds.push_back(getObjectHash(blk.sBaseTransaction));
            } else {
                txsIds.reserve(blk.vTransactionHashes.size());
            }

            if (!blk.vTransactionHashes.empty()) {
                txsIds.insert(txsIds.end(), blk.vTransactionHashes.begin(),
                              blk.vTransactionHashes.end());
            }

            std::vector<Crypto::FHash>::const_iterator ti = txsIds.begin();
            ...
            if (!txsIds.empty()) {
                ...
                for (const auto &txi : txs) {
                    ...
                    e.height = height;
                    e.timestamp = blk.uTimestamp;
                    e.transaction = *static_cast<const FTransactionPrefix *>(&txi.first);
                    e.output_indexes = txi.second;
                    e.fee = is_coinbase(txi.first)
                            ? 0
                            : getInputAmount(txi.first) - getOutputAmount(txi.first);
                }
            }
...
bool RpcServer::onGetRawTransactionPool(
        ...
        COMMAND_RPC_GET_RAW_TRANSACTIONS_FROM_POOL::response &res)
{
    ...
    for (const auto &txd : pool) {
        ...
        e.block_hash = boost::value_initialized<Crypto::FHash>();
        e.timestamp = txd.receiveTime;
        e.transaction = *static_cast<const FTransactionPrefix *>(&txd.tx);
        e.fee = txd.fee;
    }
                            ...
                            COMMAND_RPC_SEND_RAW_TX::response &res)
{
    ...
    Crypto::FHash transactionHash = Crypto::cnFastHash(tx_blob.data(), tx_blob.size());
                                 ...
                                 COMMAND_RPC_GET_BLOCKS_LIST::response &res)
{
    if (m_core.getCurrentBlockchainHeight() <= req.height) {
        throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_TOO_BIG_HEIGHT,
                                      std::string("To big height: ") + std::to_string(req.height)
                                              + ", current blockchain height = "
                                              + std::to_string(
                                                      m_core.getCurrentBlockchainHeight()) };
    }
    ...
    for (uint32_t i = req.height; i >= last_height; i--) {
        FHash block_hash = m_core.getBlockIdByHeight(static_cast<uint32_t>(i));
        FBlock blk;
        if (!m_core.getBlockByHash(block_hash, blk)) {
            throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
        ...
        size_t minerTxBlobSize = getObjectBinarySize(blk.sBaseTransaction);
        ...
        block_short.tx_count = blk.vTransactionHashes.size() + 1;
...
{
    std::list<FBlock> altBlocks;

    if (m_core.getAlternativeBlocks(altBlocks) && !altBlocks.empty()) {
        for (const auto &altBlock : altBlocks) {
            Crypto::FHash blockHash = getBlockHash(altBlock);
            uint32_t blockHeight =
                    boost::get<FBaseInput>(altBlock.sBaseTransaction.vInputs.front()).uBlockIndex;
            ...
            size_t minerTxBlobSize = getObjectBinarySize(altBlock.sBaseTransaction);
            difficulty_type blockDiff;
            m_core.getBlockDifficulty(static_cast<uint32_t>(blockHeight), blockDiff);

            BLOCK_SHORT_RESPONSE blockShort;
            blockShort.timestamp = altBlock.uTimestamp;
            ...
            blockShort.tx_count = altBlock.vTransactionHashes.size() + 1;
                            ...
                            COMMAND_RPC_GET_BLOCK_DETAILS::response &res)
{
    ...
    BLOCK_HEADER_RESPONSE_ENTRY block_header;
    res.block.height = boost::get<FBaseInput>(blk.sBaseTransaction.vInputs.front()).uBlockIndex;

    Crypto::FHash tmp_hash = m_core.getBlockIdByHeight(res.block.height);
    ...
    if (res.block.height > 0) {
        if (!m_core.getAlreadyGeneratedCoins(blk.sPreviousBlockHash, prevBlockGeneratedCoins)) {
            return false;
        }
    }

    if (res.block.height > QwertyNote::parameters::UPGRADE_HEIGHT_V1) {
        m_core.getBlockHeight(blk.sPreviousBlockHash, previousBlockHeight);
        blockTarget = blk.uTimestamp - m_core.getBlockTimestamp(previousBlockHeight);
    }
    ...
    transaction_short.hash = Common::podToHex(getObjectHash(blk.sBaseTransaction));
    transaction_short.fee = 0;
    transaction_short.amount_out = getOutsMoneyAmount(blk.sBaseTransaction);
    transaction_short.size = getObjectBinarySize(blk.sBaseTransaction);
    res.block.transactions.push_back(transaction_short);

    std::list<Crypto::FHash> missed_txs;
    std::list<FTransaction> txs;
    m_core.getTransactions(blk.vTransactionHashes, txs, missed_txs);
                                  ...
                                  COMMAND_RPC_GET_TRANSACTION_DETAILS::response &res)
{
    ...
    if (QwertyNote::getPaymentIdFromTxExtra(res.tx.vExtra, paymentId)) {
        res.txDetails.paymentId = Common::podToHex(paymentId);
    } else {
        res.txDetails.paymentId = "";
    }

    res.txDetails.extra.raw = res.tx.vExtra;

    std::vector<QwertyNote::TransactionExtraField> txExtraFields;
    parseTransactionExtra(res.tx.vExtra, txExtraFields);
    for (const QwertyNote::TransactionExtraField &field : txExtraFields) {
        if (typeid(QwertyNote::TransactionExtraPadding) == field.type()) {
            ...
            res.txDetails.extra.publicKey = getTransactionPublicKeyFromExtra(res.tx.vExtra);
                           ...
                           COMMAND_RPC_GET_POOL::response &res)
{
    ...
    for (const FTransaction tx : pool) {
        TRANSACTION_SHORT_RESPONSE transaction_short;
...
bool RpcServer::onMempoolJson(const COMMAND_RPC_GET_MEM_POOL::request &req,
...
bool RpcServer::onGetTransactionsPool(const COMMAND_RPC_GET_TRANSACTIONS_POOL::request &req,
                                      COMMAND_RPC_GET_TRANSACTIONS_POOL::response &res)
{
    ...
    for (const auto &txD : pool) {
        ...
        if (!blockchainExplorerDataBuilder.fillTransactionDetails(txD.tx, transactionDetails,
                                                                  txD.receiveTime)) {
            throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                          "Internal error: can't fill mempool tx details." };
        }
...
bool RpcServer::onGetTransactionsPoolRaw(
        const COMMAND_RPC_GET_RAW_TRANSACTIONS_FROM_POOL::request &req,
        COMMAND_RPC_GET_RAW_TRANSACTIONS_FROM_POOL::response &res)
{
    auto pool = m_core.getMemoryPool();
    ...
    for (const auto &txD : pool) {
        res.transactions.push_back(TxWithOutputGlobalIndices());
        TxWithOutputGlobalIndices &txWithOutputGlobalIndices = res.transactions.back();

        txWithOutputGlobalIndices.hash = txD.id;
        txWithOutputGlobalIndices.height = boost::value_initialized<uint32_t>();
        txWithOutputGlobalIndices.block_hash = boost::value_initialized<Crypto::FHash>();
        txWithOutputGlobalIndices.timestamp = txD.receiveTime;
        txWithOutputGlobalIndices.transaction = *static_cast<const FTransactionPrefix *>(&txD.tx);
        txWithOutputGlobalIndices.fee = txD.fee;
    }
...
bool RpcServer::onGetTransactionsPoolShort(
        const COMMAND_RPC_GET_TRANSACTIONS_POOL_SHORT::request &req,
        COMMAND_RPC_GET_TRANSACTIONS_POOL_SHORT::response &res)
{
    ...
    for (const QwertyNote::tx_memory_pool::TransactionDetails txD : pool) {
        ...
        memPoolTransaction.fee = txD.fee;
        memPoolTransaction.amountOut = getOutputAmount(txD.tx);
        memPoolTransaction.size = txD.blobSize;
        memPoolTransaction.receiveTime = txD.receiveTime;

        res.transactions.push_back(memPoolTransaction);
    }
...
bool RpcServer::onGetMixin(const FTransaction &transaction, uint64_t &mixin)
{
    mixin = 0;
...
bool RpcServer::onGetStatsByHeights(const COMMAND_RPC_GET_STATS_BY_HEIGHTS::request &req,
                                    COMMAND_RPC_GET_STATS_BY_HEIGHTS::response &res)
{
    if (m_restricted_rpc) {
        throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_RESTRICTED, "Method disabled" };
    }
    ...
    for (const uint32_t &height : req.heights) {
        ...
        entry.height = height;
        if (!m_core.getBlockEntry(height, entry.blockSize, entry.difficulty,
                                  entry.alreadyGeneratedCoins, entry.reward,
                                  entry.transactionsCount, entry.timestamp)) {
            throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                          "Internal error: can't get stats for height"
                                                  + std::to_string(height) };
        }

        entry.minFee = m_core.getMinimalFeeForHeight(height);
...
bool RpcServer::onGetStatsByHeightsRange(const COMMAND_RPC_GET_STATS_BY_HEIGHTS_RANGE::request &req,
                                         COMMAND_RPC_GET_STATS_BY_HEIGHTS_RANGE::response &res)
{
    std::chrono::steady_clock::time_point timePoint = std::chrono::steady_clock::now();

    uint32_t min = std::max<uint32_t>(req.startHeight, 1);
    uint32_t max = std::min<uint32_t>(req.endHeight, m_core.getCurrentBlockchainHeight() - 1);

    if (min >= max) {
        throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_WRONG_PARAM,
                                      "Wrong start and end heights" };
    }
    ...
    if (m_restricted_rpc) {
        uint32_t count = std::min<uint32_t>(
                std::min<uint32_t>(MAX_NUMBER_OF_BLOCKS_PER_STATS_REQUEST, max - min),
                m_core.getCurrentBlockchainHeight() - 1);
        std::vector<uint32_t> selectedHeights(count);
        double delta = (max - min) / static_cast<double>(count - 1);
        std::vector<uint32_t>::iterator i;
        double val;

        for (i = selectedHeights.begin(), val = min; i != selectedHeights.end();
             i++, val += delta) {
            *i = static_cast<uint32_t>(val);
        }

        for (const uint32_t &height : selectedHeights) {
            BLOCK_STATS_ENTRY entry;
            entry.height = height;
            if (!m_core.getBlockEntry(height, entry.blockSize, entry.difficulty,
                                      entry.alreadyGeneratedCoins, entry.reward,
        ...
        COMMAND_RPC_GET_TRANSACTIONS_BY_PAYMENT_ID::response &res)
{
    ...
    for (const FTransaction &tx : transactions) {
        TRANSACTION_SHORT_RESPONSE transaction_short;
        uint64_t amount_in = 0;
        getInputsMoneyAmount(tx, amount_in);
        uint64_t amount_out = getOutsMoneyAmount(tx);
                               ...
                               COMMAND_RPC_GET_BLOCK_HASH::response &res)
{
    ...
    Crypto::FHash blockId = m_core.getBlockIdByHeight(h);
    if (blockId == NULL_HASH) {
        throw JsonRpc::JsonRpcError {
            CORE_RPC_ERROR_CODE_TOO_BIG_HEIGHT,
            std::string("To big height: ") + std::to_string(h) + ", current blockchain height = "
                    + std::to_string(m_core.getCurrentBlockchainHeight())
        };
    }
                                   ...
                                   COMMAND_RPC_GET_BLOCK_TEMPLATE::response &res)
{
    ...
    FBlock b = boost::value_initialized<FBlock>();
    ...
    FPublicKey tx_pub_key = QwertyNote::getTransactionPublicKeyFromExtra(b.sBaseTransaction.vExtra);
...
{
    FHash currencyId = m_core.currency().genesisBlockHash();
...
uint64_t getBlockReward(const FBlock &blk)
{
    uint64_t reward = 0;
    for (const FTransactionOutput &out : blk.sBaseTransaction.vOutputs) {
        reward += out.uAmount;
    }
...
void RpcServer::fillBlockHeaderResponse(const FBlock &blk, bool orphan_status, uint64_t height,
										const FHash &hash, BLOCK_HEADER_RESPONSE_ENTRY &responce)
{
    responce.major_version = blk.uMajorVersion;
    responce.minor_version = blk.uMinorVersion;
    responce.timestamp = blk.uTimestamp;
    responce.prev_hash = Common::podToHex(blk.sPreviousBlockHash);
    responce.nonce = blk.uNonce;
                                     ...
                                     COMMAND_RPC_GET_LAST_BLOCK_HEADER::response &res)
{
    ...
    Crypto::FHash tmp_hash = m_core.getBlockIdByHeight(last_block_height);
    bool is_orphaned = last_block_hash != tmp_hash;
...
bool RpcServer::onGetBlockHeaderByHash(const COMMAND_RPC_GET_BLOCK_HEADER_BY_HASH::request &req,
                                       COMMAND_RPC_GET_BLOCK_HEADER_BY_HASH::response &res)
{
    ...
    uint64_t block_height = boost::get<FBaseInput>(blk.sBaseTransaction.vInputs.front()).uBlockIndex;
    Crypto::FHash tmp_hash = m_core.getBlockIdByHeight(block_height);
    bool is_orphaned = block_hash != tmp_hash;
...
bool RpcServer::onGetBlockHeaderByHeight(const COMMAND_RPC_GET_BLOCK_HEADER_BY_HEIGHT::request &req,
                                         COMMAND_RPC_GET_BLOCK_HEADER_BY_HEIGHT::response &res)
{
    ...
    FHash block_hash = m_core.getBlockIdByHeight(static_cast<uint32_t>(req.height));
    FBlock blk;
    if (!m_core.getBlockByHash(block_hash, blk)) {
        throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                      "Internal error: can't get block by height. Height = "
    ...
    Crypto::FHash tmp_hash = m_core.getBlockIdByHeight(req.height);
    bool is_orphaned = block_hash != tmp_hash;
                             ...
                             COMMAND_RPC_CHECK_TX_KEY::response &res)
{
    ...
    Crypto::FSecretKey tx_key = *(struct Crypto::FSecretKey *)&tx_key_hash;
    ...
    if (!Crypto::generateKeyDerivation(address.sViewPublicKey, tx_key, derivation)) {
        throw JsonRpc::JsonRpcError {
            CORE_RPC_ERROR_CODE_WRONG_PARAM,
    ...
    try {
        for (const FTransactionOutput &o : transaction.vOutputs) {
            if (o.sTarget.type() == typeid(FKeyOutput)) {
                const FKeyOutput out_key = boost::get<FKeyOutput>(o.sTarget);
                Crypto::FPublicKey pubkey;
                derivePublicKey(derivation, keyIndex, address.sSpendPublicKey, pubkey);
                if (pubkey == out_key.sPublicKey) {
                    received += o.uAmount;
                    outputs.push_back(o);
                }
            }
...
bool RpcServer::onCheckTxWithViewKey(const COMMAND_RPC_CHECK_TX_WITH_PRIVATE_VIEW_KEY::request &req,
                                     COMMAND_RPC_CHECK_TX_WITH_PRIVATE_VIEW_KEY::response &res)
{
    ...
    Crypto::FSecretKey viewKey = *(struct Crypto::FSecretKey *)&view_key_hash;
                               ...
                               COMMAND_RPC_CHECK_TX_PROOF::response &res)
{
    ...
    Crypto::FPublicKey rA;
    Crypto::FSignature sig;
    const size_t rA_len =
            Tools::Base58::encode(std::string((const char *)&rA, sizeof(Crypto::FPublicKey))).size();
    ...
    if (sizeof(Crypto::FPublicKey) != rA_decoded.size()
        || sizeof(Crypto::FSignature) != sig_decoded.size()) {
        throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_WRONG_PARAM, "Signature decoding error" };
    }
    memcpy(&rA, rA_decoded.data(), sizeof(Crypto::FPublicKey));
    memcpy(&sig, sig_decoded.data(), sizeof(Crypto::FSignature));
    ...
    Crypto::FPublicKey R = getTransactionPublicKeyFromExtra(transaction.vExtra);
    ...
    // check signature
    bool r = Crypto::checkTxProof(txid, R, address.sViewPublicKey, rA, sig);
                                    ...
                                    COMMAND_RPC_CHECK_RESERVE_PROOF::response &res)
{
    ...
    prefix_data.append((const char *)&address, sizeof(QwertyNote::FAccountPublicAddress));
    ...
    for (size_t i = 0; i < proofs.size(); ++i) {
        ...
        QwertyNote::FTransactionPrefix tx =
                *static_cast<const FTransactionPrefix *>(&transactions[i]);

        if (proof.index_in_tx >= tx.vOutputs.size()) {
            throw JsonRpc::JsonRpcError { CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
                                          "index_in_tx is out of bound" };
        }

        const FKeyOutput out_key = boost::get<FKeyOutput>(tx.vOutputs[proof.index_in_tx].sTarget);

        // get tx pub key
        Crypto::FPublicKey txPubKey = getTransactionPublicKeyFromExtra(tx.vExtra);

        // check singature for shared secret
        if (!Crypto::checkTxProof(prefix_hash, address.sViewPublicKey, txPubKey, proof.shared_secret,
        ...
        const std::vector<const Crypto::FPublicKey *> &pubs = {&out_key.sPublicKey };
        ...
        try {
            Crypto::FPublicKey pubkey;
            derivePublicKey(derivation, proof.index_in_tx, address.sSpendPublicKey, pubkey);
            if (pubkey == out_key.sPublicKey) {
                uint64_t amount = tx.vOutputs[proof.index_in_tx].uAmount;
                res.total += amount;
    ...
    Crypto::FSignature sig = proof_decoded.signature;
    if (!Crypto::checkSignature(prefix_hash, address.sSpendPublicKey, sig)) {
        res.good = false;
        return true;
    }
                                ...
                                COMMAND_RPC_VERIFY_MESSAGE::response &res)
{
    ...
    res.sig_valid = Crypto::checkSignature(hash, acc.sSpendPublicKey, s);
    res.status = CORE_RPC_STATUS_OK;
