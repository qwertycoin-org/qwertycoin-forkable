...
private:
    ...
    bool onGetPaymentId(const COMMAND_HTTP::request &req, COMMAND_HTTP::response &res);

    // json handlers
    bool onGetInfo(const COMMAND_RPC_GET_INFO::request &req, COMMAND_RPC_GET_INFO::response &res);

    bool onGetVersion(const COMMAND_RPC_GET_VERSION::request &req,
                      COMMAND_RPC_GET_VERSION::response &res);

    bool onGetHardwareInfo(const COMMAND_RPC_GET_HARDWARE_INFO::request &req,
                           COMMAND_RPC_GET_HARDWARE_INFO::response &res);

    bool onGetHeight(const COMMAND_RPC_GET_HEIGHT::request &req,
                     COMMAND_RPC_GET_HEIGHT::response &res);

    bool onGetTransactions(const COMMAND_RPC_GET_TRANSACTIONS::request &req,
                           COMMAND_RPC_GET_TRANSACTIONS::response &res);

    bool onGetTransactionsByHeights(const COMMAND_RPC_GET_TRANSACTIONS_BY_HEIGHTS::request &req,
                                    COMMAND_RPC_GET_TRANSACTIONS_BY_HEIGHTS::response &res);

    bool
    onGetRawTransactionsByHeights(const COMMAND_RPC_GET_RAW_TRANSACTIONS_BY_HEIGHTS::request &req,
                                  COMMAND_RPC_GET_RAW_TRANSACTIONS_BY_HEIGHTS::response &res);

    bool onGetRawTransactionPool(const COMMAND_RPC_GET_RAW_TRANSACTIONS_FROM_POOL::request &req,
                                 COMMAND_RPC_GET_RAW_TRANSACTIONS_FROM_POOL::response &res);

    bool onSendRawTx(const COMMAND_RPC_SEND_RAW_TX::request &req,
                     COMMAND_RPC_SEND_RAW_TX::response &res);

    bool onStartMining(const COMMAND_RPC_START_MINING::request &req,
                       COMMAND_RPC_START_MINING::response &res);

    bool onStopMining(const COMMAND_RPC_STOP_MINING::request &req,
                      COMMAND_RPC_STOP_MINING::response &res);

    bool onStopDaemon(const COMMAND_RPC_STOP_DAEMON::request &req,
                      COMMAND_RPC_STOP_DAEMON::response &res);

    bool onGetFeeAddress(const COMMAND_RPC_GET_FEE_ADDRESS::request &req,
                         COMMAND_RPC_GET_FEE_ADDRESS::response &res);

    bool onGetPeerList(const COMMAND_RPC_GET_PEER_LIST::request &req,
    ...
    bool onGetBlockDetailsByHash(const COMMAND_RPC_GET_BLOCK_DETAILS_BY_HASH::request &req,
                                 COMMAND_RPC_GET_BLOCK_DETAILS_BY_HASH::response &rsp);

    bool
    onGetBlocksHashesByTimestamps(const COMMAND_RPC_GET_BLOCKS_HASHES_BY_TIMESTAMPS::request &req,
    ...
    bool
    onGetTransactionDetailsByHash(const COMMAND_RPC_GET_TRANSACTION_DETAILS_BY_HASH::request &req,
    ...
    // json rpc
    bool onGetBlockCount(const COMMAND_RPC_GET_BLOCK_COUNT::request &req,
                         COMMAND_RPC_GET_BLOCK_COUNT::response &res);

    bool onGetBlockHash(const COMMAND_RPC_GET_BLOCK_HASH::request &req,
                        COMMAND_RPC_GET_BLOCK_HASH::response &res);

    bool onGetBlockTemplate(const COMMAND_RPC_GET_BLOCK_TEMPLATE::request &req,
                            COMMAND_RPC_GET_BLOCK_TEMPLATE::response &res);

    bool onGetCurrencyId(const COMMAND_RPC_GET_CURRENCY_ID::request &,
                         COMMAND_RPC_GET_CURRENCY_ID::response &res);

    bool onSubmitBlock(const COMMAND_RPC_SUBMIT_BLOCK::request &req,
    ...
    bool onTransactionJson(const COMMAND_RPC_GET_TRANSACTION_DETAILS::request &req,
                           COMMAND_RPC_GET_TRANSACTION_DETAILS::response &res);

    bool onPoolJson(const COMMAND_RPC_GET_POOL::request &req, COMMAND_RPC_GET_POOL::response &res);
    ...
    bool onCheckTxKey(const COMMAND_RPC_CHECK_TX_KEY::request &req,
                      COMMAND_RPC_CHECK_TX_KEY::response &res);

    bool onCheckTxWithViewKey(const COMMAND_RPC_CHECK_TX_WITH_PRIVATE_VIEW_KEY::request &req,
                              COMMAND_RPC_CHECK_TX_WITH_PRIVATE_VIEW_KEY::response &res);

    bool onCheckTxProof(const COMMAND_RPC_CHECK_TX_PROOF::request &req,
                        COMMAND_RPC_CHECK_TX_PROOF::response &res);

    bool onCheckReserveProof(const COMMAND_RPC_CHECK_RESERVE_PROOF::request &req,
                             COMMAND_RPC_CHECK_RESERVE_PROOF::response &res);

    bool onValidateAddress(const COMMAND_RPC_VALIDATE_ADDRESS::request &req,
                           COMMAND_RPC_VALIDATE_ADDRESS::response &res);

    bool onVerifyMessage(const COMMAND_RPC_VERIFY_MESSAGE::request &req,
