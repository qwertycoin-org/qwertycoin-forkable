...
JsonInputValueSerializer::JsonInputValueSerializer(Common::JsonValue &&value)
...
{
    const JsonValue *parent = m_chain.back();

    if (parent->isArray()) {
        const JsonValue &v = (*parent)[m_idxs.back()++];
        m_chain.push_back(&v);
...
{
    const JsonValue& val = *m_chain.back();
    if (val.isArray()) {
