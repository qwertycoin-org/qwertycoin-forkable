...
QwertyNote::WalletEvent makeTransactionUpdatedEvent(size_t id)
{
    QwertyNote::WalletEvent event;
    event.type = QwertyNote::WalletEventType::TRANSACTION_UPDATED;
    event.transactionUpdated.transactionIndex = id;

    return event;
}

QwertyNote::WalletEvent makeTransactionCreatedEvent(size_t id)
{
    QwertyNote::WalletEvent event;
    event.type = QwertyNote::WalletEventType::TRANSACTION_CREATED;
    event.transactionCreated.transactionIndex = id;

    return event;
}

QwertyNote::WalletEvent makeMoneyUnlockedEvent()
{
    QwertyNote::WalletEvent event;
    event.type = QwertyNote::WalletEventType::BALANCE_UNLOCKED;

    return event;
}

QwertyNote::WalletEvent makeSyncProgressUpdatedEvent(uint32_t current, uint32_t total)
{
    QwertyNote::WalletEvent event;
    event.type = QwertyNote::WalletEventType::SYNC_PROGRESS_UPDATED;
...
QwertyNote::WalletEvent makeSyncCompletedEvent()
{
    QwertyNote::WalletEvent event;
    event.type = QwertyNote::WalletEventType::SYNC_COMPLETED;
...
uint64_t calculateDonationAmount(uint64_t freeAmount,
                                        ...
                                        const Crypto::FSecretKey &viewSecretKey)
{
    ...
    m_logger(INFO, BRIGHT_WHITE)
        << "Container initialized with view secret key, public view key "
        << viewPublicKey;
}
    ...
    if (clearCachedData) {
        ...
        for (auto it = m_walletsContainer.begin(); it != m_walletsContainer.end(); ++it) {
            m_walletsContainer.modify(it, [&walletIndex](WalletRecord& wallet) {
                ...
                // NOTE: Dirty hack. Container field must be unique.
                wallet.container=reinterpret_cast<QwertyNote::ITransfersContainer *>(walletIndex++);
            });
        }
...
void WalletGreen::exportWallet(const std::string &path,
                               bool encrypt,
...
void WalletGreen::load(const std::string &path,
                       const std::string &password,
                       std::string &extra)
{
            ...
            try {
                ...
                if (!addedSpendKeys.empty()) {
                    m_logger(WARNING, BRIGHT_YELLOW)
                        << "Found addresses not saved in container cache. "
                        << "Resynchronize container";
                ...
                if (!deletedSpendKeys.empty()) {
                    m_logger(WARNING, BRIGHT_YELLOW)
                        << "Found deleted addresses saved in container cache. "
    ...
    try {
        ...
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to read output keys!! Continue without output keys: "
            << e.what();
    }
    ...
    m_state = WalletState::INITIALIZED;
    m_logger(INFO, BRIGHT_WHITE)
        << "Container loaded, view public key "
        << m_viewPublicKey
                                              ...
                                              size_t containerDataSize)
{
    ...
    chacha8(
        ...
        key,
        suffixIv,
        reinterpret_cast<char *>(encryptedContainer.data())
    );
...
void WalletGreen::deleteOrphanTransactions(const std::unordered_set<Crypto::FPublicKey> &deletedKeys)
{
    for (auto spendPublicKey : deletedKeys) {
        ...
        std::vector<size_t> updatedTransactions = deleteTransfersForAddress(
            deletedAddressString,
            deletedTransactions
        );
...
void WalletGreen::loadSpendKeys()
{
    ...
    for (size_t i = 0; i < m_containerStorage.size(); ++i) {
        ...
        wallet.pendingBalance = 0;
        // NOTE: Dirty hack. Container field must be unique.
        wallet.container = reinterpret_cast<QwertyNote::ITransfersContainer *>(i);
...
void WalletGreen::convertAndLoadWalletFile(const std::string &path,std::ifstream &&walletFileStream)
{
    ...
    for (auto spendKeys : m_walletsContainer.get<RandomAccessIndex>()) {
        m_containerStorage.push_back(encryptKeyPair(
            spendKeys.spendPublicKey,
            spendKeys.spendSecretKey,
            spendKeys.creationTimestamp
        ));
        incNextIv();
    }
...
FKeyPair WalletGreen::getAddressSpendKey(size_t index) const
{
    ...
    if (index >= m_walletsContainer.get<RandomAccessIndex>().size()) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to get address spend key: invalid address index "
            << index;
        throw std::system_error(make_error_code(std::errc::invalid_argument));
    }
    ...
    try {
...

        {
            ...
            for (auto &addressData : addressDataList) {
                auto t = std::numeric_limits<uint64_t>::max() - m_currency.blockFutureTimeLimit();
                assert(addressData.creationTimestamp <= t);
                std::string address = addWallet(
                    addressData.spendPublicKey,
                    addressData.spendSecretKey,
...
void WalletGreen::deleteAddress(const std::string &address)
{
    ...
    if (it->actualBalance != 0 || it->pendingBalance != 0) {
        m_logger(INFO, BRIGHT_WHITE)
            << "Container balance updated, actual " << m_currency.formatAmount(m_actualBalance)
            << ", pending " << m_currency.formatAmount(m_pendingBalance);
    }

    auto addressIndex = std::distance(
        m_walletsContainer.get<RandomAccessIndex>().begin(),
...
WalletTransfer WalletGreen::getTransactionTransfer(size_t transactionIndex,
                                                   size_t transferIndex) const
{
    throwIfNotInitialized();
    throwIfStopped();
    ...
    if (transferIndex >= static_cast<size_t>(std::distance(bounds.first, bounds.second))) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to get transfer: invalid transfer index "<< transferIndex
                                     ...
                                     Crypto::FSecretKey &txSecretKey)
{
    ...
    if (foundMoney < preparedTransaction.neededMoney) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to create transaction: not enough money. Needed "
    ...
    uint64_t mixIn) const
{
    ...
    auto notEnoughIt = std::find_if(
        mixinResult.begin(),
        mixinResult.end(),
        [mixIn](const QwertyNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount &ofa){
...
std::vector<WalletTransfer> WalletGreen::convertOrdersToTransfers(
...
uint64_t WalletGreen::countNeededMoney(const std::vector<WalletTransfer> &destinations,
                                       uint64_t fee) const
{
...
uint64_t WalletGreen::pushDonationTransferIfPossible(const DonationSettings &donation,
                                                     uint64_t freeAmount,
                                                     uint64_t dustThreshold,
                                                     std::vector<WalletTransfer>&destinations) const
{
    ...
    if (!donation.address.empty() && donation.threshold != 0) {
        ...
        if (donationAmount != 0) {
            ...
            m_logger(DEBUGGING)
                << "Added donation: address " << donation.address
                << ", amount " << m_currency.formatAmount(donationAmount);
        }
    }
                                            ...
                                            bool isFusion) const
{
    ...
    if (changeDestination.empty()) {
        if (sourceAddresses.size() > 1 || (sourceAddresses.empty() && m_walletsContainer.size() > 1)) {
            message =
                std::string(isFusion ? "Destination" : "Change destination")
                + " address is necessary";
            m_logger(ERROR, BRIGHT_RED)
                << message
                << ". Source addresses size=" << sourceAddresses.size()
        ...
        if (!QwertyNote::validateAddress(changeDestination, m_currency)) {
            message =
                std::string("Bad ")
                + (isFusion ? "destination" : "change destination")
                + " address: "
                + changeDestination;
            m_logger(ERROR, BRIGHT_RED) << message;
        ...
        if (!isMyAddress(changeDestination)) {
            message =
                std::string(isFusion ? "Destination" : "Change destination")
                + " address is not found in current container: "
                + changeDestination;
            m_logger(ERROR, BRIGHT_RED) << message;
            throw std::system_error(
                make_error_code(
...
void WalletGreen::validateTransactionParameters(
    const TransactionParameters &transactionParameters) const
{
    ...
    if (transactionParameters.donation.address.empty() != isZeroThreshold) {
        std::string message =
            "DonationSettings must have both address and threshold parameters filled. Address '"
...
void WalletGreen::commitTransaction(size_t transactionId)
{
    ...
    if (!ec) {
        ...
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to relay transaction: " << ec
            << ", " << ec.message()
    ...
    m_logger(INFO, BRIGHT_WHITE)
        << "Delayed transaction sent, ID " << transactionId
        << ", hash " << m_transactions[transactionId].hash;
}
...
void WalletGreen::rollbackUncommitedTransaction(size_t transactionId)
{
    ...
    m_logger(INFO, BRIGHT_WHITE)
        << "Delayed transaction rolled back, ID " << transactionId
        << ", hash " << m_transactions[transactionId].hash;
}

void WalletGreen::pushBackOutgoingTransfers(size_t txId,
...
void WalletGreen::updateTransactionStateAndPushEvent(size_t transactionId,
...
WalletGreen::TransfersMap WalletGreen::getKnownTransfersMap(size_t transactionId,
                                                            size_t firstTransferIdx) const
{
    TransfersMap result;
...
bool WalletGreen::updateAddressTransfers(size_t transactionId,
                                         size_t firstTransferIdx, const std::string &address,
                                         int64_t knownAmount,
                                         int64_t targetAmount)
{
    assert(
        (knownAmount > 0 && targetAmount > 0)
        || (knownAmount < 0 && targetAmount < 0)
        || knownAmount == 0
        || targetAmount == 0
    );
...
bool WalletGreen::updateUnknownTransfers(size_t transactionId,
                                         ...
                                         bool isOutput)
{
    ...
    if (std::abs(knownAmount) > std::abs(totalAmount)) {
        ...
        if (totalAmount == myAmount) {
            updated |= eraseTransfersByAddress(
                transactionId,
                firstTransferIdx,
                std::string(),
                isOutput
...
void WalletGreen::appendTransfer(size_t transactionId,
                                 size_t firstTransferIdx,
                                 ...
                                 int64_t amount)
{
    ...
    m_transfers.emplace(
        ...
        std::forward_as_tuple(transactionId),
        std::forward_as_tuple(transfer)
    );
}

bool WalletGreen::adjustTransfer(size_t transactionId,
                                 size_t firstTransferIdx,
                                 const std::string &address,
                                 int64_t amount)
{
...
bool WalletGreen::eraseTransfersByAddress(size_t transactionId,
                                          size_t firstTransferIdx,
                                          const std::string &address,
...
bool WalletGreen::eraseForeignTransfers(size_t transactionId,
                                        size_t firstTransferIdx,
                                        ...
                                        bool eraseOutputTransfers)
{
    return eraseTransfers(
        transactionId,
        firstTransferIdx,
        [this, &knownAddresses, eraseOutputTransfers](bool isOutput, const std::string &ta) {
        return eraseOutputTransfers == isOutput && knownAddresses.count(ta) == 0;
    });
}

std::unique_ptr<QwertyNote::ITransaction> WalletGreen::makeTransaction(
    ...
    Crypto::FSecretKey &txSecretKey)
{
    ...
    m_logger(DEBUGGING)
        << "Transaction created, hash " << tx->getTransactionHash()
...
void WalletGreen::sendTransaction(const QwertyNote::FTransaction &cryptoNoteTransaction)
{
    ...
    if (ec) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to relay transaction: " << ec << ", " << ec.message()
    ...
    bool send)
{
    ...
    Tools::QScopeExit rollbackAddingUnconfirmedTransaction([this, &transaction] {
        try {
            ...
            // Ignore any exceptions. If rollback fails then the transaction is stored
            // as unconfirmed and will be deleted after wallet relaunch during transaction
    ...
    if (send) {
        ...
        m_logger(DEBUGGING)
            << "Transaction sent to node, ID " << transactionId
            << ", hash " << transaction.getTransactionHash();
        updateTransactionStateAndPushEvent(transactionId, WalletTransactionState::SUCCEEDED);
    } else {
        ...
        m_logger(DEBUGGING)
            << "Transaction delayed, ID " << transactionId
            << ", hash " << transaction.getTransactionHash();
    }
    ...
    std::vector<COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> &mixinResult)
{
    ...
    m_node.getRandomOutsByAmounts(
        ...
        requestMixinCount,
        mixinResult,
        [&requestFinished, &mixinError, this] (std::error_code ec) {
        mixinError = ec;
        this->m_dispatcher.remoteSpawn(std::bind(
            asyncRequestCompletion,
            std::ref(requestFinished))
        );
    });
    ...
    if (mixinError) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to get random outputs: " << mixinError
            << ", " << mixinError.message();
...
uint64_t WalletGreen::selectTransfers(uint64_t neededMoney,
                                      bool dust,
                                      uint64_t dustThreshold,
                                      std::vector<WalletOuts> &&wallets,
                                      std::vector<OutputToTransfer> &selectedTransfers)
...
std::vector<WalletGreen::WalletOuts> WalletGreen::pickWallets(
...
std::vector<QwertyNote::WalletGreen::ReceiverAmounts> WalletGreen::splitDestinations(
...
void WalletGreen::prepareInputs(
    ...
    std::vector<InputInfo> &keysInfo)
{
    typedef QwertyNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry out_entry;
    ...
    for (const auto &input: selectedTransfers) {
        ...
        if(mixinResult.size()) {
            std::sort(
                mixinResult[i].outs.begin(),
                mixinResult[i].outs.end(),
                                                                  ...
                                                                  size_t count) const
{
    ...
    uint32_t blockIndex = static_cast<uint32_t>(std::distance(
        m_blockchain.get<BlockHeightIndex>().begin(),
        heightIt
    ));
...
std::vector<TransactionsInBlockInfo> WalletGreen::getTransactions(uint32_t blockIndex,
                                                                  size_t count) const
{
    throwIfNotInitialized();
    throwIfStopped();
...
std::vector<WalletTransactionWithTransfers> WalletGreen::getUnconfirmedTransactions() const
{
    ...
    auto lowerBound = m_transactions.get<BlockHeightIndex>().lower_bound(
...
{
    m_logger(ERROR, BRIGHT_RED)
        << "Synchronization error: " << ec
        << ", " << ec.message()
        << ", height " << height;
}

void WalletGreen::synchronizationProgressUpdated(uint32_t processedBlockCount,
...
void WalletGreen::onSynchronizationProgressUpdated(uint32_t processedBlockCount,
                                                   uint32_t totalBlockCount)
{
    ...
    m_logger(TRACE)
        << "onSynchronizationProgressUpdated processedBlockCount " << processedBlockCount
...
void WalletGreen::blocksRollback(uint32_t blockIndex)
{
    ...
    blockHeightIndex.erase(
        std::next(blockHeightIndex.begin(), blockIndex),
        blockHeightIndex.end()
    );
}
    ...
    const std::vector<ITransfersContainer *> &containers)
{
    ...
    for (auto container : containers) {
        ...
        // Don't move this code to the following remote spawn,
        // because it guarantees that the container has the transaction.
        uint64_t outputsAmount;
        bool found = container->getTransactionInformation(
            transactionHash,
            info,
            &inputsAmount,
            &outputsAmount
    ...
    const std::vector<ContainerAmounts> &containerAmountsList)
{
    ...
    if (isNew) {
        ...
        if (transfersUpdated) {
            m_logger(DEBUGGING)
                << "Transaction transfers updated, ID " << transactionId
                << ", hash " << m_transactions[transactionId].hash
                << ", transfers: "
                << TransferListFormatter(m_currency, getTransactionTransfersRange(transactionId));
        }
...
size_t WalletGreen::getTransactionId(const FHash &transactionHash) const
{
    ...
    if (it == m_transactions.get<TransactionIndex>().end()) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to get transaction ID: hash not found. Transaction hash "
            << transactionHash;
        throw std::system_error(make_error_code(std::errc::invalid_argument));
    }
...
void WalletGreen::transactionDeleted(ITransfersSubscription *object, const FHash &transactionHash)
{
    ...
    QwertyNote::ITransfersContainer *container = &object->getContainer();
...
void WalletGreen::addUnconfirmedTransaction(const ITransactionReader &transaction)
{
    ...
    if (ec) {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to add unconfirmed transaction: " << ec
            << ", " << ec.message();
        throw std::system_error(ec, "Failed to add unconfirmed transaction");
    }

    m_logger(DEBUGGING)
        << "Unconfirmed transaction added to BlockchainSynchronizer, hash "
        << transaction.getTransactionHash();
}
...
void WalletGreen::removeUnconfirmedTransaction(const Crypto::FHash &transactionHash)
{
    ...
    m_logger(DEBUGGING)
        << "Unconfirmed transaction removed from BlockchainSynchronizer, hash "
        << transactionHash;
}

void WalletGreen::updateBalance(QwertyNote::ITransfersContainer *container)
{
    ...
    if (updated) {
        m_walletsContainer.get<TransfersContainerIndex>().modify(
            it,
            [actual, pending](WalletRecord &wallet) {
            wallet.actualBalance = actual;
            wallet.pendingBalance = pending;
        });

        m_logger(INFO, BRIGHT_WHITE)
            << "Wallet balance updated, address "
    ...
    const std::string &destinationAddress)
{
    ...
    if (threshold <= fusionTreshold) {
        m_logger(ERROR, BRIGHT_RED)
            ...
            << ", minimum threshold "
            << m_currency.formatAmount(fusionTreshold + 1);
    ...
    if (fusionInputs.size() < m_currency.fusionTxMinInputCount()) {
        // nothing to optimize
        m_logger(WARNING, BRIGHT_YELLOW)
            << "Fusion transaction not created: nothing to optimize, threshold "
            << m_currency.formatAmount(threshold);
...
bool WalletGreen::isFusionTransaction(const WalletTransaction &walletTx) const
{
    ...
    for (const WalletRecord &wallet : walletsIndex) {
        auto c1 = wallet.container->getTransactionOutputs(
            walletTx.hash,
...
std::vector<WalletTransfer> WalletGreen::getTransactionTransfers(
    const WalletTransaction &transaction) const
{
    ...
    for (size_t i = 0; i < m_transfers.size(); ++i) {
        ...
        if ((i == m_transfers.size() - 1) || (transactionId != m_transfers[i + 1].first)) {
            ...
            if (deletedInputs != 0) {
                adjustTransfer(
                    transactionId,
                    firstTransactionTransfer,
                    "",
                    deletedInputs + unknownInputs
                );
            }
