    ...
    std::deque<std::shared_ptr<WalletLegacyEvent>> &events)
{
    ...
    try {
        ...
        createChangeDestinations(m_keys.sAddress, totalAmount, context->foundMoney, changeDts);
    ...
    std::list<FTransactionOutputInformation> &selectedTransfers)
{
    ...
    while (foundMoney < neededMoney
        ...
        if (selectOneUnmixable) {
            ...
            idx = !unusedTransfers.empty() ? popRandomValue(uRNG, unusedTransfers)
