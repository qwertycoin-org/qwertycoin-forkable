...
void Configuration::initOptions(boost::program_options::options_description &desc)
{
    desc.add_options()
        (
            ...
            "generate-container,g",
            "generate new container file with one wallet and exit"
        )(
            "view-key",
            po::value<std::string>(),
            "generate a container with this secret key view"
        )(
            "spend-key",
            po::value<std::string>(),
            "generate a container with this secret spend key"
        )(
            "mnemonic-seed",
            po::value<std::string>(),
            "generate a container with this mnemonic seed"
        )(
            "deterministic",
            "generate a container with deterministic keys. "
            "View key is generated from spend key of the first address"
        )(
            "daemon,d",
        ...
        (
            ...
            "server-root",
            po::value<std::string>(),
            "server root. The service will use it as working directory. "
            "Don't set it if don't want to change it"
        )(
            "log-level",
            po::value<size_t>(),
            "log level"
...
void Configuration::init(const boost::program_options::variables_map &options)
{
    ...
    if (registerService && unregisterService) {
        throw ConfigurationError(
            "It's impossible to use both \"register-service\" "
            "and \"unregister-service\" at the same time"
        );
    }
    ...
    if (options.count("log-level") != 0) {
        ...
        if (logLevel > Logging::TRACE) {
            std::string error =
                "log-level option must be in "
                + std::to_string(Logging::FATAL)
                +  ".."
    ...
    if (options.count("bind-address") != 0
        && (!options["bind-address"].defaulted() || bindAddress.empty())) {
    ...
    if (options.count("mnemonic-seed") != 0) {
        if (!generateNewContainer) {
            ...
            throw ConfigurationError(
                "Cannot specify import via both mnemonic seed and private keys"
            );
        }
    ...
    if (!registerService && !unregisterService) {
        if (containerFile.empty() && containerPassword.empty()) {
            throw ConfigurationError(
                "Both container-file and container-password parameters are required"
            );
        }
