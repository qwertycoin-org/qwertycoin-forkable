...
namespace {
  bool lift_up_difficulty(const CryptoNote::Currency& currency, std::vector<test_event_entry>& events,
    ...
    for (size_t i = 0; i < new_block_count; ++i) {
	  ...
	  CryptoNote::difficulty_type diffic = currency.nextDifficultyV1V2(timestamps, cummulative_difficulties);
...
bool gen_block_invalid_nonce::generate(std::vector<test_event_entry>& events) const
{
  ...
  difficulty_type diffic = m_currency.nextDifficultyV1V2(timestamps, commulative_difficulties);
  assert(1 < diffic);
...
bool gen_block_miner_tx_has_2_in::generate(std::vector<test_event_entry>& events) const
{
  ...
  Transaction tmp_tx;
  Crypto::SecretKey miner_tx_key = miner_account.getAccountKeys().spendSecretKey;
  if (!constructTransaction(miner_account.getAccountKeys(), sources, destinations, std::vector<uint8_t>(), tmp_tx, 0, miner_tx_key, m_logger))
    return false;
...
bool gen_block_miner_tx_with_txin_to_key::generate(std::vector<test_event_entry>& events) const
{
  ...
  Transaction tmp_tx;
  Crypto::SecretKey tmp_tx_key = miner_account.getAccountKeys().spendSecretKey;
...
bool gen_block_invalid_binary_format::generate(std::vector<test_event_entry>& events) const
{
  ...
  do
  {
	...
	diffic = m_currency.nextDifficultyV1V2(timestamps, cummulative_difficulties);
