...
bool test_transaction_generation_and_ring_signature()
{
  ...
  Transaction tx_rc1;
  Crypto::SecretKey tx_rc1_key = miner_acc2.getAccountKeys().spendSecretKey;
  bool r = constructTransaction(miner_acc2.getAccountKeys(), sources, destinations, std::vector<uint8_t>(), tx_rc1, 0, tx_rc1_key, logger);
  ...
  r = Crypto::checkRingSignature(pref_hash, boost::get<KeyInput>(tx_rc1.inputs[0]).keyImage,
