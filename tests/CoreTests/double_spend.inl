...
bool gen_double_spend_in_tx<txs_keeped_by_block>::generate(std::vector<test_event_entry>& events) const
{
  ...
  Crypto::SecretKey tx_1_key = bob_account.getAccountKeys().spendSecretKey;
  if (!constructTransaction(bob_account.getAccountKeys(), sources, destinations, std::vector<uint8_t>(), tx_1, 0, tx_1_key, this->m_logger))
    return false;
