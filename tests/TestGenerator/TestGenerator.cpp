...
bool constructMinerTxManually(const CryptoNote::Currency& currency, uint8_t blockMajorVersion, uint32_t height, uint64_t alreadyGeneratedCoins,
  ...
  Crypto::generateKeyDerivation(minerAddress.viewPublicKey, txkey.secretKey, derivation);
  Crypto::derivePublicKey(derivation, 0, minerAddress.spendPublicKey, outEphPublicKey);
