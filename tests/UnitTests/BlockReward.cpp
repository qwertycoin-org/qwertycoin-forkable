...
public:
    getBlockReward_and_already_generated_coins()
        : ::testing::Test(),
          m_currency(
              CryptoNote::CurrencyBuilder(m_logger).
...
TEST_F(getBlockReward_and_already_generated_coins, correctly_steps_from_reward_2_to_1) {
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() - ((UINT64_C(2) << m_currency.emissionSpeedFactor()) + 1), 1000000000)
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() -  (UINT64_C(2) << m_currency.emissionSpeedFactor())     , 1000000000)
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() - ((UINT64_C(2) << m_currency.emissionSpeedFactor()) - 1), 1000000000)
}

TEST_F(getBlockReward_and_already_generated_coins, handles_max_already_generaged_coins) {
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() - ((UINT64_C(1) << m_currency.emissionSpeedFactor()) + 1), 1000000000)
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() -  (UINT64_C(1) << m_currency.emissionSpeedFactor())     , 1000000000)
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() - ((UINT64_C(1) << m_currency.emissionSpeedFactor()) - 1), 1000000000)
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply() - 1, 1000000000)
    TEST_ALREADY_GENERATED_COINS(m_currency.moneySupply(), 1000000000)
}
...
TEST_F(getBlockReward_and_median_and_blockSize, handles_big_block_size) {
    ...
    size_t medianSize = blockSize / 2; // medianSize * 2 == blockSize
...
const uint64_t testMoneySupply = UINT64_C(16000000000);
...
TEST_F(getBlockReward_fee_and_penalizeFee_test, handles_fee_if_block_reward_is_zero_and_no_penalize_fee) {
    ...
    ASSERT_FALSE(m_blockTooBig);
    ASSERT_EQ(expectedBlockReward + fee - fee * testPenalty / 100, m_blockReward);
    ASSERT_EQ(expectedBlockReward - static_cast<int64_t>(fee * testPenalty / 100), m_emissionChange);
}
