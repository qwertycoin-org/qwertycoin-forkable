...
ICoreStub::findBlockchainSupplement(const std::vector<Crypto::Hash> &remoteBlockIds,
                                    ...
                                    uint32_t &startBlockIndex)
{
    ...
    for (uint32_t height = 0; height < static_cast<uint32_t>(std::min(blocks.size(), maxCount));
         ++height) {
...
bool ICoreStub::get_random_outs_for_amounts(
...
bool ICoreStub::handle_incoming_tx(CryptoNote::BinaryArray const &tx_blob,
...
void ICoreStub::set_random_outs(
        const CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response &resp, bool result)
{
    randomOuts = resp;
...
bool ICoreStub::getPoolChanges(const Crypto::Hash &tailBlockId,
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash &tailBlockId,
                                   const std::vector<Crypto::Hash> &knownTxsIds,
...
void ICoreStub::getPoolChanges(const std::vector<Crypto::Hash> &knownTxsIds,
                               std::vector<CryptoNote::Transaction> &addedTxs,
                               std::vector<Crypto::Hash> &deletedTxsIds)
{
}
...
bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash> &block_ids, uint64_t timestamp,
                            ...
                            std::vector<CryptoNote::BlockFullInfo> &entries)
{
    // stub
    return true;
}
...
bool ICoreStub::queryBlocksDetailed(const std::vector<Crypto::Hash> &knownBlockHashes,
...
void ICoreStub::getTransactions(const std::vector<Crypto::Hash> &txs_ids,
...
bool ICoreStub::getBackwardBlocksSizes(uint32_t fromHeight, std::vector<size_t> &sizes,
                                       size_t count)
{
    return true;
}
...
bool ICoreStub::getBlockReward(uint8_t blockMajorVersion, size_t medianSize,
                               size_t currentBlockSize, uint64_t alreadyGeneratedCoins,
                               uint64_t fee, uint64_t &reward, int64_t &emissionChange,
                               uint32_t height, uint64_t blockTarget)
{
    return true;
}

bool ICoreStub::scanOutputkeysForIndices(
        const CryptoNote::KeyInput &txInToKey,
...
bool ICoreStub::getBlockCumulativeDifficulty(uint32_t height,
                                             CryptoNote::difficulty_type &difficulty)
{
    return true;
}

bool ICoreStub::getBlockContainingTx(const Crypto::Hash &txId, Crypto::Hash &blockId,
                                     uint32_t &blockHeight)
{
    ...
    blockHeight =
            boost::get<CryptoNote::BaseInput>(blockIter->second.baseTransaction.inputs.front())
                    .blockIndex;
    return true;
}
...
void ICoreStub::addBlock(const CryptoNote::Block &block)
{
    ...
    blockHashByTxHashIndex.emplace(
            std::make_pair(CryptoNote::getObjectHash(block.baseTransaction), hash));
...
bool ICoreStub::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd,
...
bool ICoreStub::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd,
...
bool ICoreStub::getTransactionsByPaymentId(const Crypto::Hash &paymentId,
                                           std::vector<CryptoNote::Transaction> &transactions)
{
    return true;
}
...
bool ICoreStub::handleIncomingTransaction(const CryptoNote::Transaction &tx,
...
bool ICoreStub::addMessageQueue(
        CryptoNote::MessageQueue<CryptoNote::BlockchainMessage> &messageQueuePtr)
{
    return true;
}

bool ICoreStub::removeMessageQueue(
        CryptoNote::MessageQueue<CryptoNote::BlockchainMessage> &messageQueuePtr)
{
    return true;
}
