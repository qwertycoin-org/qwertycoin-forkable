...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  ...
  Crypto::generateKeys(ignoredPublicKey, ignoredSectetKey);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    ...
    coreStub.handleIncomingTransaction(tx, CryptoNote::getObjectHash(tx), CryptoNote::getObjectBinarySize(tx), tvc, keptByBlock, node.getLastLocalBlockHeight(), false);
